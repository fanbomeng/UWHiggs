...
////      .AddSyst(cb, "CMS_eff_btag_13TeV", "lnN", SystMap<>::init(1.051));
  ...
  cb.cp().process({"TT"}).bin_id({2750})
      .AddSyst(cb, "CMS_eff_btag_13TeV", "lnN", SystMap<>::init(1.0245));


  cb.cp().process({"T"}).bin_id({2200})
...
//  TFile output("HMuTau_mutauhad_2016.input.root", "RECREATE");
  ...
  for (auto b : bins) {
    for (auto m : masses) {
      ...
      cb.cp().mass({"750", "*"}).bin({"HMuTau_mutauhad_1750_2016","HMuTau_mutauhad_2750_2016"}).WriteDatacard(folder + "/"+lumi+"/"+"HMuTau_mutauhad_combined_2016"+textbinbybin+"_m750_"+lumi+".txt", output);
