...
//  TFile output("HMuTau_mutauhad_2016.input.root", "RECREATE");
  ...
  for (auto b : bins) {
    for (auto m : masses) {
      ...
      cb.cp().mass({"450", "*"}).bin({"HMuTau_mutauhad_1450_2016","HMuTau_mutauhad_2450_2016"}).WriteDatacard(folder + "/"+lumi+"/"+"HMuTau_mutauhad_combined_2016"+textbinbybin+"_m450_"+lumi+".txt", output);
