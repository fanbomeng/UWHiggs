...
////      .AddSyst(cb, "CMS_eff_btag_13TeV", "lnN", SystMap<>::init(1.051));
  ...
  cb.cp().process({"TT"}).bin_id({2600})
      .AddSyst(cb, "CMS_eff_btag_13TeV", "lnN", SystMap<>::init(1.0245));


  cb.cp().process({"T"}).bin_id({2600})
...
//  TFile output("HMuTau_mutauhad_2016.input.root", "RECREATE");
  ...
  for (auto b : bins) {
    for (auto m : masses) {
      ...
      cb.cp().mass({"600", "*"}).bin({"HMuTau_mutauhad_1600_2016","HMuTau_mutauhad_2600_2016"}).WriteDatacard(folder + "/"+lumi+"/"+"HMuTau_mutauhad_combined_2016"+textbinbybin+"_m600_"+lumi+".txt", output);
