...
////      .AddSyst(cb, "CMS_eff_btag_13TeV", "lnN", SystMap<>::init(1.051));
  ...
  cb.cp().process({"TT"}).bin_id({2900})
      .AddSyst(cb, "CMS_eff_btag_13TeV", "lnN", SystMap<>::init(1.0245));


  cb.cp().process({"T"}).bin_id({2900})
      .AddSyst(cb, "CMS_eff_btag_13TeV", "lnN", SystMap<>::init(1.0211));



  cb.cp().AddSyst(cb,"theo_gauss","lnN",SystMap<process,bin_id>::init
    ({"LFV"},{1900},1.022)
    ({"LFV"},{2900},1.022)
    );


  cb.cp().AddSyst(cb,"theo_pdf_alpa","lnN",SystMap<process,bin_id>::init
    ({"LFV"},{1900},1.046)
    ({"LFV"},{2900},1.046)
    );
...
//
  cb.cp().process(ch::JoinStr({{"Fakes"}}))
     .AddSyst(cb, "CMS_TauFakeRate_p0_dm01_L_13TeV","shape",SystMap<>::init(1.));
  cb.cp().process(ch::JoinStr({{"Fakes"}}))
     .AddSyst(cb, "CMS_TauFakeRate_p1_dm01_L_13TeV","shape",SystMap<>::init(1.));
  cb.cp().process(ch::JoinStr({{"Fakes"}}))
     .AddSyst(cb, "CMS_TauFakeRate_p0_dm01_M_13TeV","shape",SystMap<>::init(1.));
  cb.cp().process(ch::JoinStr({{"Fakes"}}))
     .AddSyst(cb, "CMS_TauFakeRate_p0_dm01_H_13TeV","shape",SystMap<>::init(1.));
  cb.cp().process(ch::JoinStr({{"Fakes"}}))
     .AddSyst(cb, "CMS_TauFakeRate_p0_dm10_L_13TeV","shape",SystMap<>::init(1.));
  cb.cp().process(ch::JoinStr({{"Fakes"}}))
     .AddSyst(cb, "CMS_TauFakeRate_p0_dm10_M_13TeV","shape",SystMap<>::init(1.));
  cb.cp().process(ch::JoinStr({{"Fakes"}}))
     .AddSyst(cb, "CMS_TauFakeRate_p0_dm10_H_13TeV","shape",SystMap<>::init(1.));
//
//  cb.cp().process(ch::JoinStr({{"Fakes"}}))
...
//  TFile output("HMuTau_mutauhad_2016.input.root", "RECREATE");
  ...
  for (auto b : bins) {
    for (auto m : masses) {
      ...
      //cb.cp().mass({"125", "*"}).bin({"HMuTau"+name+"_mutauhad_0_2016","HMuTau"+name+"_mutauhad_1_2016","HMuTau"+name+"_mutauhad_3_2016"}).WriteDatacard(folder + "/"+lumi+"/"+"HMuTau_"+name+"_mutauhad_combined_2016"+textbinbybin+"_m125_"+lumi+".txt", output);
      cb.cp().mass({"900", "*"}).bin({"HMuTau_mutauhad_1900_2016","HMuTau_mutauhad_2900_2016"}).WriteDatacard(folder + "/"+lumi+"/"+"HMuTau_mutauhad_combined_2016"+textbinbybin+"_m900_"+lumi+".txt", output);
