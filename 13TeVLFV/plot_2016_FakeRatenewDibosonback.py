...
#mmtLoosewwold = make_histo(savedir,"WW_TuneCUETP8M1_13TeV-pythia8",channelLoose,var)
#mmtLoosewzold = make_histo(savedir,"WZ_TuneCUETP8M1_13TeV-pythia8",channelLoose,var)
#mmtLoosezzold = make_histo(savedir,"ZZ_TuneCUETP8M1_13TeV-pythia8",channelLoose,var)
...
#mmtTightwwold = make_histo(savedir,"WW_TuneCUETP8M1_13TeV-pythia8",channelTight,var)
#mmtTightwzold = make_histo(savedir,"WZ_TuneCUETP8M1_13TeV-pythia8",channelTight,var)
#mmtTightzzold = make_histo(savedir,"ZZ_TuneCUETP8M1_13TeV-pythia8",channelTight,var)
...
if ("m1_m2" not in var):
#        fakeRate=ROOT.TEfficiency(fakeRate,fakeRate);
...
#        fakeRateData=ROOT.TEfficiency(fakeRateData,dataLoose);
...
fakeRate.SetMaximum(0)
fakeRateData.SetMaximum(0)
fakeRate.GetXaxis().SetRangeUser(0,2.5)
fakeRateData.GetXaxis().SetRangeUser(0,2.5)
fakeRate.Draw("ep")
...
legend = ROOT.TLegend(0.5763333,0.3142857,0.8990606,0.5006803)
...
if ("Eta" in var):
  ...
  if ("m3Eta" in var):
     fakeRate.GetXaxis().SetTitle("#mu |#eta|")
  if ("tEta" in var):
     fakeRate.GetXaxis().SetTitle("#tau |#eta|")
...
if ("Pt" in var):
 #fakeFit = ROOT.TF1("adsf","[0]+[1]*x+[2]*x*x+[3]*x*x*x+[4]*x*x*x*x",40,200)
  fakeFit = ROOT.TF1("adsf","[0]",30,200)
 #fakeFit = ROOT.TF1("adsf","[0]+[1]*x+[2]*x*x",-2.5,2.5)
  fakeRateData.Fit(fakeFit,"R")
