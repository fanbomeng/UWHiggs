...
parser.add_argument(
    ...
    action="store",
    dest="xlabel",
    default="Collinear Mass [GeV]",
    help="Nam for the  axis")
parser.add_argument(
    "--higgsSF",
    type=int,
    action="store",
...
def add_lumi():
    ...
    lumi.SetTextFont (   42 )
    lumi.AddText("2016, 36.8 fb^{-1} (13 TeV)")
    return lumi
...
#        #if (10*s/((b+0.09*b*0.09*b)**0.5) > 0.5):
        ...
        if (forceBlinding>0 and 2*s/(0.0000001+s+b)**0.5 > 0.2):
    ...
    if "lfv" in cat and "_3_" in cat:
        categ.AddText(catMap[channel]+", 2 jets gg-enriched")
    if "lfv" in cat and "_4_" in cat:
        categ.AddText(catMap[channel]+", 2 jets VBF-enriched")
    if "_ch1" in cat:
        categ.AddText(catMap[channel]+", 0 jet")
    if "_ch2" in cat:
        categ.AddText(catMap[channel]+", 1 jet")
    ...
    if isLog:
       ...
       c.SaveAs("plots/"+cat+".pdf")
       c.SaveAs("plots/"+cat+".png")
