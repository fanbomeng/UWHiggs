...
tmptotal_1=sysneed+sysneed_I+sysneed_III+sysneed_II
#tmptotal_1=sysneed+sysneed_I+sysneed_III
for massnameI in masspointbaseI:
   f = open('Run_Cutbased'+massnameI+'_rename.sh', 'w')
   for i in tmptotal_1:
         for j in base:
                f.write('python plot_13TeV_vbfbined_QCD_36_highmass_5new.py  LFV_MiniAODVtrial_sys/AnalyzeLFVMuTau/ collMass_type1 '+j+'200'+ '    0  '+i+'    tPt0   0 ' +massnameI + '\n')
   f.close()

f = open('Run_Cutbased200_450_rename.sh', 'w')
for i in tmptotal_1:
      for j in base:
             f.write('python plot_13TeV_vbfbined_QCD_36_highmass_5new.py  LFV_MiniAODVtrial_sys/AnalyzeLFVMuTau/ collMass_type1 '+j+'200'+ '    0  '+i+'    tPt0   0  450 '  + '\n')
f.close()

for massnameII in masspointbaseII:
   f = open('Run_Cutbased'+massnameII+'_rename.sh', 'w')
   for i in tmptotal_1:
         for j in base:
                f.write('python plot_13TeV_vbfbined_QCD_36_highmass_5new.py  LFV_MiniAODVtrial_sys/AnalyzeLFVMuTau/ collMass_type1 '+j+'450'+ '    0  '+i+'    tPt0   0 ' +massnameII + '\n')
   f.close()



f = open('Run_Cutbased200_rename.sh', 'a')
