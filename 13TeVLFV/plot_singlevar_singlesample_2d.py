...
def do_binbybin2d(histo,file_str,lowBound,highBound): #fill empty bins and negtive content bins to a scaled number, but detail ?
	metafile = lumidir + file_str+"_weight.log"
        f = open(metafile).read().splitlines()
        nevents = float((f[0]).split(': ',1)[-1])
        xsec = eval("XSec."+file_str.replace("-","_"))
      ...
      #                  break
		...
		if fillEmptyBins: #fill empty bins
			if histo.GetBinContent(i,j) <= 0:
				histo.SetBinContent(i,j,0.001/nevents*xsec*JSONlumi)   
				histo.SetBinError(i,j,1.8/nevents*xsec*JSONlumi)
		else:
                        if histo.GetBinContent(i,j) < 0:
                                histo.SetBinContent(i,j,0.001/nevents*xsec*JSONlumi)
                                histo.SetBinError(i,j,1.8/nevents*xsec*JSONlumi)
def make_histo(savedir,file_str, channel,var,lumidir,lumi,isData=False,):     #get histogram from file, properly weight histogram
		...
		histo.Scale(lumi/efflumi) 
                #print "for sample %s" %file_str
                #print "the effectlumi  %f  and final scale  %f"   %(efflumi,lumi/efflumi) 
	else:	
...
if "vbf_gg" in channel:
	rootdir = "mutau_vbf_gg"
        numberJet='2'
if "vbf_vbf" in channel:
	rootdir = "mutau_vbf_vbf"
        numberJet='2'
if "boost" in channel:
	rootdir = "mutau_boost"
        numberJet='1'
...
#Sample_list=['Data','DYJets','ZTauTauJets','TT','WJets','VBF_LFV','GluGlu_LFV','VBFHToTauTau','GluGluHToTauTau','WW','WZ','ZZ','ST_tW_antitop','ST_tW_top']
Sample_list=['Data','DYJets','VBF_LFV','GluGlu_LFV']
...
hist9=type(wjets)
hist10=type(wjets)
histdatafinal=type(wjets)
histmcfinal=type(wjets)
W1jets=type(wjets)
W2jets=type(wjets)
W3jets=type(wjets)
W4jets=type(wjets)
...
#          if i==0:
   if sampleused=="WJets":
                ...
                W1jets = make_histo(savedir,Sampletable["W1Jets"],channel,var,lumidir,lumi)
                W2jets = make_histo(savedir,Sampletable["W2Jets"],channel,var,lumidir,lumi)
                W3jets = make_histo(savedir,Sampletable["W3Jets"],channel,var,lumidir,lumi)
                W4jets = make_histo(savedir,Sampletable["W4Jets"],channel,var,lumidir,lumi)
          #      do_binbybin2d(W1jets,Sampletable["W1Jets"],binlowx,binhigx)
          #      do_binbybin2d(W2jets,Sampletable["W2Jets"],binlowx,binhigx)
          #      do_binbybin2d(W3jets,Sampletable["W3Jets"],binlowx,binhigx)
          #      do_binbybin2d(W4jets,Sampletable["W4Jets"],binlowx,binhigx)
                histlist[0].Add(W1jets)
                histlist[0].Add(W2jets)
                histlist[0].Add(W3jets)
...
#             histdatafinal=histlist[0]
 ...
 #            histmcfinal=histlist[0] 
            ...
            # do_binbybin2d(histlist[4],Sampletable["ZTauTau4Jets"],binlowx,binhigx)
             ...
             histlist[0].Add(histlist[4])
             do_binbybin2d(histlist[0],Sampletable[sampleused],binlowx,binhigx)
  #           histmcfinal.Add(histlist[0])
   else:
             histlist[0] = make_histo(savedir,Sampletable[sampleused],channel,var,lumidir,lumi)
             do_binbybin2d(histlist[0],Sampletable[sampleused],binlowx,binhigx)
   #          histmcfinal.Add(histlist[0])
 #  histlist[0].Rebin(binwidth)
 #  histlist[0].SetLineColor(colorlist[0])
 #  histlist[0].SetLineWidth(3)
#legend.AddEntry(histdatafinal,"Data")
#******#norm=gghmutau125_0.GetEntries()
#******#gghmutau125_0.Scale(1.00/norm);
   histlist[0].SetTitle("")
#******histdatafinal.GetXaxis().SetTitle(xlabel)
#******histdatafinal.GetXaxis().SetNdivisions(510)
#******histdatafinal.GetXaxis().SetLabelSize(0.035)
#******histdatafinal.GetXaxis().SetRangeUser(0,xRange)
#******histdatafinal.GetYaxis().SetTitle(ylabel)
#******histdatafinal.GetYaxis().SetTitleOffset(1.40)
#******histdatafinal.SetMinimum(0)
#******#   if normalize==1:
#******#      normalizetag="normalized"
#******#      scale = 1.00/(histlist[0].Integral());
#******#      histlist[0].Scale(scale)
#******#      histlist[0].GetYaxis().SetLabelSize(0.035)
#******#      histlist[0].SetMaximum(1)
#******#      histlist[0].SetMinimum(0)
   histlist[0].Draw("hsames COLZ")
#histdatafinal.Draw("hsames COLZ")
   outfile_name = savedir+sampleused+"_"+channel+"_"+var+"_"+shiftStr+opcut
