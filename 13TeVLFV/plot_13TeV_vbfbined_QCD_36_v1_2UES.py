...
#                      histo.SetBinContent(i,0.0001)
               #       histo.SetBinError(i,0.0)
        if lowBoundMC<=lowBound:
           lowBoundMC=lowBound
        else:
           for i in range(lowBound,lowBoundMC+1):
                   if fillEmptyBins: #fill empty bins
                           if histo.GetBinContent(i) <= 0:
                                   histo.SetBinContent(i,0.00001)
                           #       histo.SetBinError(i,1.8/nevents*xsec*JSONlumi)
                                   histo.SetBinError(i,0.0000001)
        if highBoundMC>=highBound:
           highBoundMC=highBound
        else:
           for i in range(highBoundMC,highBound+1):
                   if fillEmptyBins: #fill empty bins
                           if histo.GetBinContent(i) <= 0:
                                   histo.SetBinContent(i,0.00001)
                           #       histo.SetBinError(i,1.8/nevents*xsec*JSONlumi)
                                   histo.SetBinError(i,0.0000001)
        if lowBoundMC<=highBoundMC:
           for i in range(lowBoundMC, highBoundMC+1):
                   if fillEmptyBins: #fill empty bins
                           if histo.GetBinContent(i) <= 0:
                                   histo.SetBinContent(i,0.00001)
                           #       histo.SetBinError(i,1.8/nevents*xsec*JSONlumi)
