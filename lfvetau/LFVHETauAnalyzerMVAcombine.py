...
class LFVHETauAnalyzerMVAcombine(MegaBase):
    tree = 'et/final/Ntuple'
    def __init__(self, tree, outfile, **kwargs):
        logging.debug('LFVHETauAnalyzerMVAcombine constructor')
        self.channel='ET'
        super(LFVHETauAnalyzerMVAcombine, self).__init__(tree, outfile, **kwargs)
            ...
            for name, shifted in sys_effects:
             ...
             #       passes_full_selection = True 
                if shifted.njets == 0 :
                    ...
                    if shifted.tPt < 40: continue
                    if shifted.ePt < 33: continue
                    if row.tMtToPfMet >40 : continue
                    selection_categories.append((name, '1', 'selected'))
                    passes_full_selection = True 
                elif shifted.njets == 2 :
                    if shifted.tPt < 35: continue
                    if shifted.ePt < 35: continue
                    if row.tMtToPfMet >50 : continue
                    if row.vbfMass < 500 : continue
                    if row.vbfDeta < 2.3 : continue
                    selection_categories.append((name, '2', 'selected'))
                    passes_full_selection = True 
