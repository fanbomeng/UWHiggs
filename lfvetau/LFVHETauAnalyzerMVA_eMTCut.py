...
class LFVHETauAnalyzerMVA_eMTCut(MegaBase):
    tree = 'et/final/Ntuple'
    def __init__(self, tree, outfile, **kwargs):
        self.channel='ET'
        super(LFVHETauAnalyzerMVA_eMTCut, self).__init__(tree, outfile, **kwargs)
              ...
              #      jetlist.extend([(int(jn_jes_plus), str(int(jn_jes_plus))+'_jes_plus'), (int(jn_jes_minus), str(int(jn_jes_plus))+'_jes_minus')])
                for jet in jetlist:
                    ...
                    if jet[0] == 0 :
                        ...
                        if row.ePt < 46 : continue
                        if deltaPhi(row.ePhi, row.tPhi) < 2 : continue
                        if row.tMtToPFMET > 77 : continue
                    if jet[0] == 1 :
                        if row.tPt < 31: continue 
                        if row.ePt < 42 : continue
                        if row.tMtToPFMET > 70 : continue
                    if jet[0] == 2 :
                        if row.tPt < 35: continue 
                        if row.ePt < 40 : continue # no cut as only electrons with pt>30 are in the ntuples
                        if row.tMtToPFMET > 60 : continue
                        if row.vbfMass < 400: continue
                        if row.vbfDeta < 3.2 : continue
                 ...
                 #   if jet[0] == 0 :
                 #       if row.tPt < 30: continue 
                 #       if row.ePt < 51 : continue
                 #       if deltaPhi(row.ePhi, row.tPhi) < 2.3 : continue
                 #       if row.tMtToPFMET > 60 : continue
                 #   if jet[0] == 1 :
                 #       if row.tPt < 32: continue 
                 #       if row.ePt < 51 : continue
                 #       if row.tMtToPFMET > 55 : continue
                 #   if jet[0] == 2 :
                 #       if row.tPt < 36: continue 
                 #       if row.ePt < 56 : continue # no cut as only electrons with pt>30 are in the ntuples
                 #       if row.tMtToPFMET > 44 : continue
                 #       if row.vbfMass < 448 : continue
