#include <cmath>
#include "formats10.h" 
void makePlots_optmass(){

  TString pathResults1 = "results/newNtuple_10Oct/LFVHETauAnalyzerMVA_eMTCut/";
...
//  TString pathPlots3="os/gg/ept30/";  
  ...
  //TFile *fdataA= new TFile(pathName(pathResults1, "data_SingleElectron_Run2012A_22Jan2013_v1.root"));
 // TFile *fdataB= new TFile(pathName(pathResults1, "data_SingleElectron_Run2012B_22Jan2013_v1.root"));
 // TFile *fdataC= new TFile(pathName(pathResults1, "data_SingleElectron_Run2012C_22Jan2013_v1.root"));
  //TFile *fdataD= new TFile(pathName(pathResults1, "data_SingleElectron_Run2012D_22Jan2013_v1.root"));
...
// for(int i=0;i<=3;i++)
// { 
    jet0tune(pathPlots2,pathInputs,"h_collmass_pfmet",0,fLFVvbf,fW1Jets, fW2Jets, fW3Jets, fW4Jets, fGGHTT,fTTJetsFLMGD, fTTJetsSLMGD, fT_tZ2star, fT_twZ2star, fTbar_tZ2star, fTbar_twZ2star,fWWJets, fWZJets, fZ0jetsTT, fZ1jetsTT, fZ2jetsTT, fZ3jetsTT, fZ4jetsTT, fZ0jetsLL, fZ1jetsLL, fZ2jetsLL, fZ3jetsLL, fZ4jetsLL, fZZjets, fZetauA, fZetauB, fZetauC, fZetauD,fLFVgg);

// }
// jet0tune(pathplots0,"tPtcut",0, fLFVvbf,fW1Jets,LFVvbfLumi,W1JetsLumi,DataLumi);
}
void jet0tune(TString path,TString pathInputs,char * variable,int rebin, TFile *fLFVvbf,TFile *fW1Jets,TFile * fW2Jets,TFile * fW3Jets,TFile * fW4Jets,TFile * fGGHTT,TFile *fTTJetsFLMGD,TFile * fTTJetsSLMGD,TFile * fT_tZ2star,TFile * fT_twZ2star,TFile *  fTbar_tZ2star,TFile * fTbar_twZ2star,TFile *fWWJets,TFile * fWZJets,TFile * fZ0jetsTT,TFile * fZ1jetsTT,TFile * fZ2jetsTT,TFile * fZ3jetsTT,TFile * fZ4jetsTT,TFile * fZ0jetsLL,TFile * fZ1jetsLL,TFile * fZ2jetsLL,TFile * fZ3jetsLL,TFile * fZ4jetsLL,TFile * fZZjets,TFile * fZetauA,TFile * fZetauB,TFile * fZetauC,TFile * fZetauD,TFile * fLFVgg)

{
  ...
  //TH1F *hdataA= (TH1F*) fdataA->Get(pathName(path,variable));
 // TH1F *hdataB= (TH1F*) fdataB->Get(pathName(path,variable));
 // TH1F *hdataC= (TH1F*) fdataC->Get(pathName(path,variable));
 // TH1F *hdataD= (TH1F*) fdataD->Get(pathName(path,variable));
...
//  hLFV->SetLineWidth(2);
  hLFVvbf->SetLineColor(2);
  hLFVvbf->SetLineStyle(2);
  hLFVvbf->SetLineWidth(2);
  //hLFVvbf->SetFillColor(1); 
  hLFVvbf->Scale(100*DataLumi/LFVvbfLumi);
  ...
  hTTJetsFLMGD->SetLineColor(28);
  hTTJetsFLMGD->SetFillColor(28);        
  hTTJetsFLMGD->Scale(DataLumi/TTJetsFLMGDLumi); 
  hTTJetsSLMGD->SetLineColor(28);
  hTTJetsSLMGD->SetFillColor(28);        
  ...
  hZ0jetsLL->SetLineColor(3);         
  hZ0jetsLL->SetFillColor(3); 
  hZ0jetsLL->Scale(DataLumi/Z0JetsLLLumi);        
  hZ1jetsLL->SetLineColor(3);         
  hZ1jetsLL->SetFillColor(3); 
  hZ1jetsLL->Scale(DataLumi/Z1JetsLLLumi);       
  hZ2jetsLL->SetLineColor(3);         
  hZ2jetsLL->SetFillColor(3); 
  hZ2jetsLL->Scale(DataLumi/Z2JetsLLLumi);      
  hZ3jetsLL->SetLineColor(3);         
  hZ3jetsLL->SetFillColor(3); 
  hZ3jetsLL->Scale(DataLumi/Z3JetsLLLumi);     
  hZ4jetsLL->SetLineColor(3);         
  hZ4jetsLL->SetFillColor(3); 
  hZ4jetsLL->Scale(DataLumi/Z4JetsLLLumi);    
  hZZjets->SetLineColor(6);         
  hZZjets->SetFillColor(6); 
  hZZjets->Scale(DataLumi/ZZJetsLumi);   
  hZetauA->SetLineColor(5);         
  hZetauA->SetFillColor(5); 
  hZetauA->Scale(DataLumi/ZetauLumiA);       
  hZetauB->SetLineColor(5);         
  hZetauB->SetFillColor(5); 
  hZetauB->Scale(DataLumi/ZetauLumiB);      
  hZetauC->SetLineColor(5);         
  hZetauC->SetFillColor(5); 
  hZetauC->Scale(DataLumi/ZetauLumiC);     
  hZetauD->SetLineColor(5);         
  hZetauD->SetFillColor(5); 
  hZetauD->Scale(DataLumi/ZetauLumiD);  
  hLFVgg->SetLineColor(2); 
  hLFVgg->SetLineWidth(2); 
//  hLFVgg-> SetFillColor(2); 
  hLFVgg->Scale(100*DataLumi/LFVggLumi);     
//  hLFVgg->add(hLFVvbf);
...
//  TH1F * signalall= new TH1F("h1","h1 title",32,0,320);
  //TH1F * data1= new TH1F("data1","data1 title",32,0,320);
  //TH1F * data2= new TH1F("data2","data2 title",32,0,320);
  //TH1F * data3= new TH1F("data3","data3 title",32,0,320);
  //addhist(data1,hdataA,hdataB);
  //addhist(data2,hdataC,hdataD);
  //addhist(data3,data1,data2);
  //data3->SetLineColor(1);
//  signalall->SetLineColor(4);
...
//  leg->SetTextSize(0.03679144);
 ...
 // leg->AddEntry(signalall,"LFV","pl");
  ...
  c1->cd();
//  c1->SetGrid();
// addhist(signalall,hLFVvbf,hLFVgg);
//  std::cout<<hLFVgg->GetSize()<<std::endl;
//  TH1F *try1= Add(hLFVgg,hLFVvbf,1,1);
  //histos->GetStack()->Last()->Draw();
 // histos->GetHistogram()->GetXaxis()->SetTitle("collinear M GeV"); 
 // histos->GetHistogram()->GetYaxis()->SetTitle("Events"); 
 histos->Draw("HIST");
 histos->GetXaxis()->SetTitle("collinear M GeV");
 histos->GetYaxis()->SetTitle("Events");
 histos->SetTitle("CMS preliminary");
  hLFVvbf->Draw("hist same");
 // signalall->Draw("hist SAME ");
  leg->AddEntry(hLFVgg,"LFV gg","pl");
  leg->AddEntry(hLFVvbf,"LFV vbf","pl");
  leg->AddEntry(hW1Jets,"WJets","pl");
  leg->AddEntry(hGGHTT,"GGH to TT","pl");
  leg->AddEntry(hTTJetsFLMGD,"TTjet LepDecay","pl");
  leg->AddEntry(hT_tZ2star,"T chanel","pl");
  leg->AddEntry(hT_tZ2star,"T chanel","pl");
  leg->AddEntry(hWWJets,"ww wz jets","pl");
  leg->AddEntry(hZ0jetsTT,"Z jets","pl");
//  leg->AddEntry(hZ0jetsLL,"Z jets LL","pl");
//  leg->AddEntry(hZZjets,"Z Zjets","pl");
  leg->AddEntry(hZetauA,"Z etau","pl");
  //leg->AddEntry(data3,"signal e","pl");
  hLFVgg->Draw("hist SAME ");
 // data3->Draw("hist same");
  leg->Draw("same");
  c1->Modified();
  char  ss1[100];
  sprintf(ss1,"plots/massplots/newlosscut/2jets/%s.png",variable);
  c1->Print(ss1);
...
//   gSB->Draw("AP");
 //  gSB->Draw();
 //  char  ss1[100];
 //  sprintf(ss1,"plots/round1/1jets/%s.png",variable);
...
int addhist(TH1F * h1,TH1F *h2, TH1F *h3)
  {
   int c1,c2,c3;
     c1= h2->GetSize();
     c2= h3->GetSize();
   if(c1!=c2)
    std::cout<<"different size of bin"<<std::endl;
  for(int i=0;i<=c1-1;i++)
   h1->SetBinContent(i,h2->GetBinContent(i)+h3->GetBinContent(i));
