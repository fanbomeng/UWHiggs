...
if not args.no_plots:
   ...
   for sign, proc, thr, njet in itertools.product(signs, processtype, threshold, jets):
      ...
      for var, xlabel, rebin in histo_info:
         ...
         plotter.plot_with_bkg_uncert(path, var, rebin, xlabel,leftside=False, show_ratio=True, ratio_range=1.,sort=True, obj=['e'], data=True)

            
         plotter.save(var)
            
      plotter.set_subdir(os.path.join('embedded', path+'/vbf_deta4.0'))if embedded else plotter.set_subdir(path+'/vbf_deta4.0')
      ...
      for var, xlabel, rebin in histo_info:
         ...
         logging.debug("Plotting %s/%s" % (path, var) )
         plotter.pad.SetLogy(False)
         plotter.plot_with_bkg_uncert(path+'/vbf_deta4.0', var, rebin, xlabel,
...
if not args.no_shapes:
   signal_region = 'os/gg/ept30/%s/vbf_deta4.0'
   jets_names = [
         #  ('0', 'gg0etau'  , 1),
#           ('1', 'boostetau', 1),
           ('2', 'vbfetau'  , 5),
