...
class LFVHETauAnalyzerMVAfullscan(MegaBase):
    tree = 'et/final/Ntuple'
    def __init__(self, tree, outfile, **kwargs):
        logging.debug('LFVHETauAnalyzerMVAfullscan constructor')
        self.channel='ET'
        super(LFVHETauAnalyzerMVAfullscan, self).__init__(tree, outfile, **kwargs)
        ...
        self.hfunc   = { #maps the name of non-trivial histograms to a function to get the proper value, the function MUST have two args (evt and weight). Used in fill_histos later
            ...
            'tPtcut' : coll_mass,
            'ePtcut' : coll_mass,
            'deltaPhicut' : coll_mass,
            'tMtToPFMETcut' : coll_mass,
            'vbfMasscut' : coll_mass,
            'vbfDetacut' : coll_mass,
...
#                     ['etLoose', 'etLoose/Up', 'etLoose/Down'] 
        ...
        signs =['os', 'ss']
        jetN = ['0', '1', '2', '3'] 
         #[''.join(i) for i in itertools.product(['0', '1', '2', '3'], self.systematics['jes'])]
...
#            self.book(f, "jetVeto30", "Number of jets, p_{T}>30", 5, -0.5, 4.5) 
    ...
    def fill_histos2(self,histname,cutvalue,folder_str, row, weight, filter_label = ''):
        '''fills histograms'''
        #find all keys matching
        for attr, value in self.histo_locations[folder_str].iteritems():
            name = attr
            ...
            if value.InheritsFrom('TH2'):
                ...
                if attr in self.hfunc:
                    try:
                        result, out_weight = self.hfunc['h_collmass_pfmet'](row, weight)
                    except Exception as e:
                        raise RuntimeError("Error running function %s. Error: \n\n %s" % (attr, str(e)))
                    if out_weight is None:
                        if name=="tPtcut":
                           value.Fill(cutvalue) #saves you when filling NTuples!
                    else:
                        value.Fill(cutvalue, out_weight )
                else:
                    value.Fill( getattr(row,attr), weight ) if weight is not None else value.Fill( getattr(row,attr) )
        return None


    def fill_histos3(self,histname,cutvalue,folder_str, row, weight, filter_label = ''):
        '''fills histograms'''
        ...
        if out_weight is None:
            if histname=="tPtcut":
              #  value.Fill(cutvalue) #saves you when filling NTuples!
                print "hahahahha"
                histos[folder_str+'/tPtcut'].Fill(cutvalue)
         #  else:
          #      histos[folder_str+'/tPtcut'].Fill(cutvalue,out_weight)
            #    value.Fill(cutvalue, out_weight )
        else:
            #    value.Fill( getattr(row,attr), weight ) if weight is not None else value.Fill( getattr(row,attr) )
            if histname=="tPtcut":
                histos[folder_str+'/tPtcut'].Fill(cutvalue,out_weight)
            if histname=="ePtcut":
                histos[folder_str+'/ePtcut'].Fill(cutvalue,out_weight)
            if histname=="deltaPhicut":
                histos[folder_str+'/deltaPhicut'].Fill(cutvalue,out_weight)
            if histname=="tMtToPFMETcut":
                histos[folder_str+'/tMtToPFMETcut'].Fill(cutvalue,out_weight)
            if histname=="vbfMasscut":
                histos[folder_str+'/vbfMasscut'].Fill(cutvalue,out_weight)
            if histname=="vbfDetacut":
                histos[folder_str+'/vbfDetacut'].Fill(cutvalue,out_weight)
          #  print "hahahahha"
        return None
...
#                )
     ...
     #       logging.debug('Passed Vetoes') 
            jet0panel=[('tPtcut',1,1,30,70),('ePtcut',1,1,30,70),('deltaPhicut',1,1,20,35),('tMtToPFMETcut',1,1,20,80)]
            jet1panel=[('tPtcut',1,1,30,70),('ePtcut',1,1,30,70),('tMtToPFMETcut',1,1,20,80)]
            jet2panel=[('tPtcut',1,1,30,70),('ePtcut',1,1,30,70),('tMtToPFMETcut',1,1,20,80),('vbfMasscut',1,4,300,700),('vbfDetacut',1,1,20,60)]
            jet0refer=(30,46,2.6,70)
            jet1refer=(30,41,65)
            jet2refer=(30,51,70,400,2.3)
                    ...
                    for nbjet0,jetwhole0 in enumerate(jet0panel): 
                          if nbjet0==0 and jetwhole0[1]==1 :
                              for j in range(jetwhole0[3],jetwhole0[4],jetwhole0[2]): 
                                      ...
                                      passes_full_selection = True 
                                      self.fill_histos3(jetwhole0[0],j,foldertofill, row, weight_map[''])
                          ...
                          if nbjet0==2 and jetwhole0[1]==1 :
                              for j in range(jetwhole0[3],jetwhole0[4],jetwhole0[2]): 
                                      ...
                                      if row.tMtToPfMet >jet0refer[3]: continue
                                      passes_full_selection = True 
                                      self.fill_histos3(jetwhole0[0],j/10.0,foldertofill, row, weight_map[''])
                    ...
                    for nbjet1,jetwhole1 in enumerate(jet1panel): 
                          if nbjet1==0 and jetwhole1[1]==1 :
                              for j in range(jetwhole1[3],jetwhole1[4],jetwhole1[2]): 
                                   ...
                                   #   selection_categories.append((name, '1', 'selected'))
                                      passes_full_selection = True 
                                      self.fill_histos3(jetwhole1[0],j,foldertofill, row, weight_map[''])
                       ...
                       for nbjet2,jetwhole2 in enumerate(jet2panel):
                          if nbjet2==0 and jetwhole2[1]==1 :
                              for j in range(jetwhole2[3],jetwhole2[4],jetwhole2[2]):
                                      ...
                                      passes_full_selection = True 
                                      self.fill_histos3(jetwhole2[0],j,foldertofill, row, weight_map[''])
                          ...
                          if nbjet2==4 and jetwhole2[1]==1 :
                              for j in range(jetwhole2[3],jetwhole2[4],jetwhole2[2]):
                                      ...
                                      passes_full_selection = True 
                                      self.fill_histos3(jetwhole2[0],j/10.0,foldertofill, row, weight_map[''])
                  #   selection_categories.append((name, '2', 'selected'))
...
#                continue
   ...
   #                 weight_map[i] *= mc_weight
        ...
        #        self.fill_histos2("tPtcut",10,"os/gg/ept30/1/", row, weight_to_use)
                #self.fill_histos2("tPtcut",10,dir_name, row, weight_to_use)
