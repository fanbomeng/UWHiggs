...
class AnalyzeLFVMuTau(MegaBase):
    ...
    def __init__(self, tree, outfile, **kwargs):
     ...
     #   print self.is_data
        ...
        self.is_OneJet=('W1JetsToLNu' in targetor('DY1JetsToLL' in target)or('ZTauTau1JetsToLL' in target))
        self.is_TwoJet=('W2JetsToLNu' in targetor('DY2JetsToLL' in target)or('ZTauTau2JetsToLL' in target))
        self.is_ThreeJet=('W3JetsToLNu' in targetor('DY3JetsToLL' in target)or('ZTauTau3JetsToLL' in target))
        self.is_FourJet=('W4JetsToLNu' in targetor('DY4JetsToLL' in target)or('ZTauTau4JetsToLL' in target))
...
##        histos[name+'/eVetoMVAIso'].Fill(row.eVetoMVAIso, weight)
    ...
    def gg(self,row):
       ...
       if row.tPt < 30:  #was 35
           return False
       if row.tMtToPfMet_type1 > 65:  #was 50
           return False
       if row.jetVeto30!=0:
           return False
       return True
    ...
    def boost(self,row):
          ...
          if row.tPt < 30:  #was 40
                return False
          if row.tMtToPfMet_type1 > 75: #was 35
                return False
          return True
    ...
    def vbf(self,row):
       ...
       # if row.mPt < 30:
       # 	return False
        if row.tMtToPfMet_type1 > 55: #was 35
                return False
        if row.jetVeto30<2:  
            return False
	if(row.vbfNJets<2):
