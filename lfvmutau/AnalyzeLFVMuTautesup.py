...
#	metE = row.type1_pfMet_shiftedPt_TauEnUp
#	metPhi = row.type1_pfMet_shiftedPhi_TauEnUp
...
class AnalyzeLFVMuTautesup(MegaBase):
    tree = 'mt/final/Ntuple'
    #tree = 'New_Tree'

    def __init__(self, tree, outfile, **kwargs):
        super(AnalyzeLFVMuTautesup, self).__init__(tree, outfile, **kwargs)
...
#           tmpMet=self.Metcorected.CorrectByMeanResolution(row.type1_pfMet_shiftedPt_TauEnUp*math.cos(row.type1_pfMet_shiftedPhi_TauEnUp),row.type1_pfMetEt*math.sin(row.type1_pfMetPhi),row.genpX,row.genpY,row.vispX,row.vispY,int(round(row.jetVeto30)))#,self.pfmetcorr_ex,self.pfmetcorr_ey)
...
#	histos[name+'/mDPhiToPfMet_tDPhiToPfMet'].Fill(abs(row.mDPhiToPfMet_TauEnUp),abs(row.tDPhiToPfMet_TauEnUp),weight)
#	histos[name+'/mDPhiToPfMet_ggdeltaphi'].Fill(abs(row.mDPhiToPfMet_TauEnUp),deltaPhi(row.mPhi, row.tPhi),weight)
#	histos[name+'/tDPhiToPfMet_ggdeltaphi'].Fill(abs(row.tDPhiToPfMet_TauEnUp),deltaPhi(row.mPhi, row.tPhi),weight)
#	histos[name+'/tDPhiToPfMet_tMtToPfMet_type1'].Fill(abs(row.tDPhiToPfMet_TauEnUp),row.tMtToPfMet_TauEnUp,weight)
...
#           return False
       if row.tPt < 30:  #was 35   #newcuts30
           return False
#       if row.tMtToPfMet_TauEnUp > 105:  #was 50   #newcuts65
#           return False
       if self.ls_recoilC and MetCorrection:
...
#	    return False
  ...
  #          return False
    ...
    def vbf_gg(self,row):
        ...
        if row.mPt < 25:   #was 40    #newcut 25
       		return False
   #     if row.tMtToPfMet_TauEnUp > 105: #was 35   #newcuts 55
   #             return False
        if self.ls_recoilC and MetCorrection:
            if self.tMtToPfMet_type1MetC > 105:  #was 50   #newcuts65
               return False
