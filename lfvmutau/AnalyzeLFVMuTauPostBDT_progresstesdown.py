...
def fullMT(mupt,taupt , muphi, tauphi, row, sys='none'):
        ...
        met = row.type1_pfMet_shiftedPt_TauEnDown
        metphi = row.type1_pfMet_shiftedPhi_TauEnDown
...
class AnalyzeLFVMuTauPostBDT_progresstesdown(MegaBase):
    tree = 'mt/final/Ntuple'
    #tree = 'New_Tree'

    def __init__(self, tree, outfile, **kwargs):
        super(AnalyzeLFVMuTauPostBDT_progresstesdown, self).__init__(tree, outfile, **kwargs)
...
#          histos['jetPt'].Fill(1,1)
    def MetCorrectionSet(self,row):
           if self.ls_Wjets:
              self.tmpMet=self.Metcorected.CorrectByMeanResolution(row.type1_pfMet_shiftedPt_TauEnDown*math.cos(row.type1_pfMet_shiftedPhi_TauEnDown),row.type1_pfMetEt*math.sin(row.type1_pfMetPhi),row.genpX,row.genpY,row.vispX,row.vispY,int(round(row.jetVeto30))+1)#,self.pfmetcorr_ex,self.pfmetcorr_ey)
           else:
              self.tmpMet=self.Metcorected.CorrectByMeanResolution(row.type1_pfMet_shiftedPt_TauEnDown*math.cos(row.type1_pfMet_shiftedPhi_TauEnDown),row.type1_pfMetEt*math.sin(row.type1_pfMetPhi),row.genpX,row.genpY,row.vispX,row.vispY,int(round(row.jetVeto30)))#,self.pfmetcorr_ex,self.pfmetcorr_ey)
           ...
           print "Mass from Tuple %f and from Cal %f" %(row.type1_pfMet_shiftedPt_TauEnDown*math.cos(row.type1_pfMet_shiftedPhi_TauEnDown),self.tmpMet[0])
...
#        histos[name+'/mPtavColMass'].Fill(row.mPt/row.m_t_collinearmass_TauEnDown, weight)
        histos[name+'/mEta'].Fill(row.mEta, weight)
        histos[name+'/mMtToPfMet_type1'].Fill(row.mMtToPfMet_TauEnDown,weight)
...
#        histos[name+'/tPtavColMass'].Fill(row.tPt/row.m_t_collinearmass_TauEnDown, weight)
        ...
        if self.ls_recoilC and MetCorrection:
           ...
           histos[name+'/tMtToPfMet_type1'].Fill(row.tMtToPfMet_TauEnDown,weight)
...
#        histos[name+'/tCharge'].Fill(row.tCharge, weight)
        ...
        histos[name+'/tMass'].Fill(row.tMass_TauEnDown,weight)
#        histos[name+'/tLeadTrackPt'].Fill(row.tLeadTrackPt,weight)
        ...
        if self.ls_recoilC and MetCorrection:
           ...
           histos[name+'/tDPhiToPfMet_type1'].Fill(abs(row.tDPhiToPfMet_TauEnDown),weight)

        if self.ls_recoilC and MetCorrection:
           histos[name+'/collMass_type1'].Fill(self.collMass_type1MetC,weight)
     #      print "Mass from Tuple %f and from Cal %f" %(row.tMtToPfMet_TauEnDown,self.tMtToPfMet_type1MetC) 
     #      print "Mass from Tuple %f and from Cal %f" %(row.m_t_collinearmass_TauEnDown,self.collMass_type1MetC) 
        else:
           histos[name+'/collMass_type1'].Fill(row.m_t_collinearmass_TauEnDown,weight)
...
#        histos[name+'/collMass_type1MetC'].Fill(self.collMass_type1(row,tmpMet[0],tmpMet[1]),weight)
    #    print row.m_t_collinearmass_TauEnDown
        ...
        histos[name+'/type1_pfMetEt'].Fill(row.type1_pfMet_shiftedPt_TauEnDown,weight)

        histos[name+'/m_t_Mass'].Fill(row.m_t_Mass_TauEnDown,weight)
        ...
        histos[name+'/mDPhiToPfMet_type1'].Fill(abs(row.mDPhiToPfMet_TauEnDown),weight)
        histos[name+'/mPhiMtPhi'].Fill(deltaPhi(row.mPhi,row.tPhi),weight)
        histos[name+'/mPhiMETPhiType1'].Fill(deltaPhi(row.mPhi,row.type1_pfMet_shiftedPhi_TauEnDown),weight)
        histos[name+'/tPhiMETPhiType1'].Fill(deltaPhi(row.tPhi,row.type1_pfMet_shiftedPhi_TauEnDown),weight)
...
#	return True
    ...
    def gg(self,row):
       ...
       if row.tPt < 30:  #was 35   #newcuts30
           return False
       if row.tMtToPfMet_TauEnDown > 75:  #was 50   #newcuts65
           return False
       if abs(row.tDPhiToPfMet_TauEnDown)>3.0:
           return False
       if row.jetVeto30!=0:
           return False
    ...
    def boost(self,row):
          ...
          if row.tPt < 30:  #was 40  #newcut 30
                return False
          if row.tMtToPfMet_TauEnDown > 105: #was 35   #newcuts 75
                return False
          if abs(row.tDPhiToPfMet_TauEnDown)>3.0:
                return False
          if row.bjetCISVVeto30Loose:
    ...
    def vbf(self,row):
       ...
       # if row.mPt < 30:
       # 	return False
        if row.tMtToPfMet_TauEnDown > 75: #was 35   #newcuts 55
                return False
        if row.jetVeto30<2:  
            return False
#	if(row.vbfNJets<2):
...
#	    return False
    ...
    def vbf_vbf(self,row):
        ...
        if row.mPt < 25:   #was 40    #newcut 25
                return False
  #      if row.tMtToPfMet_TauEnDown > 85: #was 35   #newcuts 55
  #              return False
...
#                return False
        if self.ls_recoilC and MetCorrection:
            ...
            if row.tMtToPfMet_TauEnDown > 85:  #was 50   #newcuts65
               return False
#        if row.jetVeto30<2:
#            return False
#        if(row.vbfNJets30<2):
...
#            return False
    ...
    def WjetsEnrich(self,row):
        if (row.tMtToPfMet_TauEnDown>60 and row.mMtToPfMet_TauEnDown>80):
            return True
        return False
...
#            self.var_d_0  ={'mPt_':row.mPt,'tPt_':row.tPt,'mMtToPfMet_type1_':row.mMtToPfMet_TauEnDown,'tMtToPfMet_type1_':row.tMtToPfMet_TauEnDown,'m_t_DPhi_':row.m_t_DPhi,'tDPhiToPfMet_type1_':row.tDPhiToPfMet_TauEnDown,'type1_pfMetEt_':row.type1_pfMet_shiftedPt_TauEnDown,'jetVeto30_':row.jetVeto30,'vbfMass_':-0.1,'vbfDeta_':-0.1,'deltaeta_m_t_':row.mEta-row.tEta} 
            self.var_d_0  ={'mPt_':row.mPt,'tPt_':row.tPt,'tMtToPfMet_type1_':row.tMtToPfMet_TauEnDown,'m_t_DPhi_':abs(row.m_t_DPhi),'tDPhiToPfMet_type1_':row.tDPhiToPfMet_TauEnDown,'type1_pfMetEt_':row.type1_pfMet_shiftedPt_TauEnDown,'deltaeta_m_t_':abs(row.mEta-row.tEta),'m_t_collinearmass_':row.m_t_collinearmass_TauEnDown}
            self.var_d_1  =self.var_d_0 
            self.var_d_2  =self.var_d_0
           # self.var_d_2  ={'mPt_':row.mPt,'tPt_':row.tPt,'mMtToPfMet_type1_':row.mMtToPfMet_TauEnDown,'tMtToPfMet_type1_':row.tMtToPfMet_TauEnDown,'m_t_DPhi_':row.m_t_DPhi,'tDPhiToPfMet_type1_':row.tDPhiToPfMet_TauEnDown,'type1_pfMetEt_':row.type1_pfMet_shiftedPt_TauEnDown,'jetVeto30_':row.jetVeto30,'vbfMass_':row.vbfMass/2600.0,'vbfDeta_':row.vbfDeta/10.0,'deltaeta_m_t_':row.mEta-row.tEta} 
            #self.var_d_2  ={'mMtToPfMet_type1_':row.mMtToPfMet_TauEnDown,'tMtToPfMet_type1_':row.tMtToPfMet_TauEnDown,'m_t_DPhi_':row.m_t_DPhi,'tDPhiToPfMet_type1_':row.tDPhiToPfMet_TauEnDown,'type1_pfMetEt_':row.type1_pfMet_shiftedPt_TauEnDown,'jetVeto30_':row.jetVeto30,'vbfMass_':row.vbfMass,'vbfDeta_':row.vbfDeta,'deltaeta_m_t_':row.mEta-row.tEta} 
#            if row.jetVeto30==0:
...
#               MVA2=self.functor(**self.var_d_2)
            ...
            #        if MVA0>-0.01: 
                    ...
                    if RUN_OPTIMIZATION:
                       for  i in optimizer_new.compute_regions_0jet(row.tPt, row.mPt, deltaPhi(row.mPhi,row.tPhi),abs(row.tDPhiToPfMet_TauEnDown),row.tMtToPfMet_TauEnDown):
             ...
             #       if MVA0>-0.01:
                    ...
                    if RUN_OPTIMIZATION:
                       for  i in optimizer_new.compute_regions_1jet(row.tPt, row.mPt,deltaPhi(row.mPhi,row.tPhi),abs(row.tDPhiToPfMet_TauEnDown),row.tMtToPfMet_TauEnDown):
              ...
              #      if MVA0>-0.01:
                    ...
                    if RUN_OPTIMIZATION:
                          for  i in optimizer_new.compute_regions_2jettight(row.tPt, row.mPt,row.tMtToPfMet_TauEnDown,row.vbfMass,row.vbfDeta):
                    ...
                    if RUN_OPTIMIZATION:
                       for  i in optimizer_new.compute_regions_2jetloose(row.tPt, row.mPt,row.tMtToPfMet_TauEnDown,row.vbfMass,row.vbfDeta):
...
#              print row.m_t_collinearmass_TauEnDown
             ...
             #     self.fill_histos(row,'gg',False)
              ...
              if (row.jetVeto30==2) :
#                  if RUN_OPTIMIZATION:
#                     for  i in optimizer.compute_regions_2jet(row.tPt, row.mPt,row.tMtToPfMet_TauEnDown,row.vbfMass,row.vbfDeta):
...
#                 self.fill_histos(row,'notIsoNotWeighted',False)
           ...
           #          self.fill_histos(row,'ggNotIso',True)
                 if row.jetVeto30==1:
                  #  # if RUN_OPTIMIZATION:
                  #  #    for  i in optimizer.compute_regions_1jet(row.tPt, row.mPt,row.tMtToPfMet_TauEnDown):
...
#                 self.fill_histos(row,'notIsoNotWeighted',False)
                 ...
                 if  row.jetVeto30==0:
                    # #if RUN_OPTIMIZATION:
                       # for  i in optimizer.compute_regions_0jet(row.tPt, row.mPt, deltaPhi(row.mPhi,row.tPhi),row.tMtToPfMet_TauEnDown):
                    # #   for  i in optimizer.compute_regions_0jet(row.tPt, row.mPt, deltaPhi(row.mPhi,row.tPhi),abs(row.mDPhiToPfMet_TauEnDown),abs(row.tDPhiToPfMet_TauEnDown),row.tMtToPfMet_TauEnDown):
