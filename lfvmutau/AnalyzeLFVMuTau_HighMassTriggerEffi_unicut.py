...
class AnalyzeLFVMuTau_HighMassTriggerEffi_unicut(MegaBase):
    tree = 'mt/final/Ntuple'
    #tree = 'New_Tree'

    def __init__(self, tree, outfile, **kwargs):
        super(AnalyzeLFVMuTau_HighMassTriggerEffi_unicut, self).__init__(tree, outfile, **kwargs)
    ...
    def begin(self):
        ...
        if not self.light :
           ...
           names=["preselectionSS","notIsoSS","notIsoSSM","notIsoSSMT","preselection0Jet","notIso0Jet","notIsoM0Jet","notIsoMT0Jet","preselection1Jet","notIso1Jet","notIsoM1Jet","notIsoMT1Jet"]
           for i in self.highMassbase:
               for j in self.highMass:
                   names.append(i+j)
           print names
...
#                 print "comes in as USE111111111111111111"
    ...
    #    if (MassPoint=='125') or (MassPoint=='def'):
       #    if row.mPt < 40 or row.tMtToPfMet_type1 >70 or row.tPt<35:   #was45     #newcuts 25 
        #     return False 
   #     elif  MassPoint=='200':
        if row.mPt < 60 or row.tPt<35 or row.tMtToPfMet_type1 >65:   #was45     #newcuts 25 
             return False 
   #     elif  MassPoint=='300':
   #       if row.mPt < 85 or row.tPt<40 or row.tMtToPfMet_type1 >90:   #was45     #newcuts 25 
   #          return False 
   #     elif  MassPoint=='450':
   #       if row.mPt < 120 or row.tPt<50  or row.tMtToPfMet_type1 >110:   #was45     #newcuts 25 
   #          return False 
   #     elif  MassPoint=='600':
...
#            return False
  ...
  #           return False 
  #      elif  MassPoint=='200':
        if row.mPt < 55 or row.tPt<35 or row.tMtToPfMet_type1 >60:   #was45     #newcuts 25 
             return False 
 #       elif  MassPoint=='300':
 #         if row.mPt < 75 or row.tPt<40  or row.tMtToPfMet_type1 >95:   #was45     #newcuts 25 
 #            return False 
 #       elif  MassPoint=='450':
 #         if row.mPt < 115 or row.tPt<50 or row.tMtToPfMet_type1 >120:   #was45     #newcuts 25 
 #            return False 
 #       elif  MassPoint=='600':
