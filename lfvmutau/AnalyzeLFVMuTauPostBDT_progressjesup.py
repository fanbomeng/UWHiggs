...
def fullMT(mupt,taupt , muphi, tauphi, row, sys='none'):
        ...
        met = row.type1_pfMet_shiftedPt_JetEnUp
        metphi = row.type1_pfMet_shiftedPhi_JetEnUp
...
def getFakeRateFactorFANBOPt(row, fakeset):
     ...
     if fakeset=="1stUp":
        if  row.tDecayMode==0:
            fTauIso=0.21753+0.00724482
        if  row.tDecayMode==1:
            fTauIso=0.21068+0.00406504
        if  row.tDecayMode==10:
            fTauIso=0.18071+0.00448410
     if fakeset=="1stDown":
        if  row.tDecayMode==0:
            fTauIso=0.21753-0.00724482
        if  row.tDecayMode==1:
...
class AnalyzeLFVMuTauPostBDT_progressjesup(MegaBase):
    tree = 'mt/final/Ntuple'
    #tree = 'New_Tree'

    def __init__(self, tree, outfile, **kwargs):
        super(AnalyzeLFVMuTauPostBDT_progressjesup, self).__init__(tree, outfile, **kwargs)
...
#            self.book(names[x], "vbfNJets30", "g", 5, -0.5, 4.5)
    ...
    def fakeRateMethod(self,row,fakeset,faketype):
        ...
        if faketype=="muonfake":
           ...
           #return getFakeRateFactormuon(row,fakeset)
       ...
       #    return getFakeRateFactormuon(row,fakeset)*getFakeRateFactorFANBOPt(row,fakeset)
...
#          histos['jetPt'].Fill(1,1)
    def MetCorrectionSet(self,row):
           if self.ls_Wjets:
              self.tmpMet=self.Metcorected.CorrectByMeanResolution(row.type1_pfMet_shiftedPt_JetEnUp*math.cos(row.type1_pfMet_shiftedPhi_JetEnUp),row.type1_pfMetEt*math.sin(row.type1_pfMetPhi),row.genpX,row.genpY,row.vispX,row.vispY,int(round(row.jetVeto30_JetEnUp))+1)#,self.pfmetcorr_ex,self.pfmetcorr_ey)
           else:
              self.tmpMet=self.Metcorected.CorrectByMeanResolution(row.type1_pfMet_shiftedPt_JetEnUp*math.cos(row.type1_pfMet_shiftedPhi_JetEnUp),row.type1_pfMetEt*math.sin(row.type1_pfMetPhi),row.genpX,row.genpY,row.vispX,row.vispY,int(round(row.jetVeto30_JetEnUp)))#,self.pfmetcorr_ex,self.pfmetcorr_ey)
           ...
           print "Mass from Tuple %f and from Cal %f" %(row.type1_pfMet_shiftedPt_JetEnUp*math.cos(row.type1_pfMet_shiftedPhi_JetEnUp),self.tmpMet[0])
...
#        histos[name+'/mPtavColMass'].Fill(row.mPt/row.m_t_collinearmass_JetEnUp, weight)
        histos[name+'/mEta'].Fill(row.mEta, weight)
        histos[name+'/mMtToPfMet_type1'].Fill(row.mMtToPfMet_JetEnUp,weight)
...
#        histos[name+'/tPtavColMass'].Fill(row.tPt/row.m_t_collinearmass_JetEnUp, weight)
        ...
        if self.ls_recoilC and MetCorrection:
           ...
           histos[name+'/tMtToPfMet_type1'].Fill(row.tMtToPfMet_JetEnUp,weight)
...
#        histos[name+'/tLeadTrackPt'].Fill(row.tLeadTrackPt,weight)
        ...
        if self.ls_recoilC and MetCorrection:
           ...
           histos[name+'/tDPhiToPfMet_type1'].Fill(abs(row.tDPhiToPfMet_JetEnUp),weight)

        if self.ls_recoilC and MetCorrection:
           histos[name+'/collMass_type1'].Fill(self.collMass_type1MetC,weight)
     #      print "Mass from Tuple %f and from Cal %f" %(row.tMtToPfMet_JetEnUp,self.tMtToPfMet_type1MetC) 
     #      print "Mass from Tuple %f and from Cal %f" %(row.m_t_collinearmass_JetEnUp,self.collMass_type1MetC) 
        else:
           histos[name+'/collMass_type1'].Fill(row.m_t_collinearmass_JetEnUp,weight)
...
#        histos[name+'/collMass_type1MetC'].Fill(self.collMass_type1(row,tmpMet[0],tmpMet[1]),weight)
    #    print row.m_t_collinearmass_JetEnUp
        ...
        histos[name+'/type1_pfMetEt'].Fill(row.type1_pfMet_shiftedPt_JetEnUp,weight)
        ...
        histos[name+'/mDPhiToPfMet_type1'].Fill(abs(row.mDPhiToPfMet_JetEnUp),weight)
        histos[name+'/mPhiMtPhi'].Fill(deltaPhi(row.mPhi,row.tPhi),weight)
        histos[name+'/mPhiMETPhiType1'].Fill(deltaPhi(row.mPhi,row.type1_pfMet_shiftedPhi_JetEnUp),weight)
        histos[name+'/tPhiMETPhiType1'].Fill(deltaPhi(row.tPhi,row.type1_pfMet_shiftedPhi_JetEnUp),weight)
        histos[name+'/tDecayMode'].Fill(row.tDecayMode, weight)
        histos[name+'/vbfMass'].Fill(row.vbfMass_JetEnUp, weight)
        histos[name+'/vbfDeta'].Fill(row.vbfDeta_JetEnUp, weight)
#        histos[name+'/vbfNJets30'].Fill(row.vbfNJets_JetEnUp30, weight)
    def presel(self, row):
...
#	return True
    ...
    def gg(self,row):
       ...
       if row.tPt < 30:  #was 35   #newcuts30
           return False
       if row.tMtToPfMet_JetEnUp > 75:  #was 50   #newcuts65
           return False
       if abs(row.tDPhiToPfMet_JetEnUp)>3.0:
           return False
       if row.jetVeto30_JetEnUp!=0:
           return False
    ...
    def boost(self,row):
          if row.jetVeto30_JetEnUp!=1:
            return False
          if row.mPt < 25:  #was 35    #newcuts 25
                return False
          if row.tPt < 30:  #was 40  #newcut 30
                return False
          if row.tMtToPfMet_JetEnUp > 105: #was 35   #newcuts 75
                return False
          if abs(row.tDPhiToPfMet_JetEnUp)>3.0:
                return False
          if row.bjetCISVVeto30Loose:
    ...
    def vbf(self,row):
       ...
       # if row.mPt < 30:
       # 	return False
        if row.tMtToPfMet_JetEnUp > 75: #was 35   #newcuts 55
                return False
        if row.jetVeto30_JetEnUp<2:  
            return False
#	if(row.vbfNJets_JetEnUp<2):
#	    return False
	if(abs(row.vbfDeta_JetEnUp)<0.3):   #was 2.5    #newcut 2.0
	    return False
     #   if row.vbfMass_JetEnUp < 200:    #was 200   newcut 325
#	    return False
        if row.vbfJetVeto30_JetEnUp > 0:
            return False
...
#	if(row.vbfNJets_JetEnUp<2):
#	    return False
	if(abs(row.vbfDeta_JetEnUp)>3.2):   #was 2.5    #newcut 2.0
	    return False
        if row.vbfMass_JetEnUp > 500:    #was 20   newcut 240
	    return False
        if row.vbfJetVeto30_JetEnUp > 0:
            return False
        if row.bjetCISVVeto30Medium:
    ...
    def vbf_vbf(self,row):
        ...
        if row.mPt < 25:   #was 40    #newcut 25
                return False
  #      if row.tMtToPfMet_JetEnUp > 85: #was 35   #newcuts 55
  #              return False
...
#                return False
        if self.ls_recoilC and MetCorrection:
            ...
            if row.tMtToPfMet_JetEnUp > 85:  #was 50   #newcuts65
               return False
#        if row.jetVeto30_JetEnUp<2:
#            return False
#        if(row.vbfNJets_JetEnUp30<2):
#            return False
        if(abs(row.vbfDeta_JetEnUp)<3.5 or (row.vbfMass_JetEnUp <= 550)):   #was 2.5    #newcut 2.0
            return False
#        if row.vbfMass_JetEnUp > 240:    #was 200   newcut 325
#           return False
#        if row.vbfJetVeto30_JetEnUp > 0:
#            return False
#        if  row.bjetCISVVeto30Medium:
#            return False
    ...
    def WjetsEnrich(self,row):
        if (row.tMtToPfMet_JetEnUp>60 and row.mMtToPfMet_JetEnUp>80):
            return True
        return False
...
#            self.var_d_0  ={'mPt_':row.mPt,'tPt_':row.tPt,'mMtToPfMet_type1_':row.mMtToPfMet_JetEnUp,'tMtToPfMet_type1_':row.tMtToPfMet_JetEnUp,'m_t_DPhi_':row.m_t_DPhi,'tDPhiToPfMet_type1_':row.tDPhiToPfMet_JetEnUp,'type1_pfMetEt_':row.type1_pfMet_shiftedPt_JetEnUp,'jetVeto30_':row.jetVeto30_JetEnUp,'vbfMass_':-0.1,'vbfDeta_':-0.1,'deltaeta_m_t_':row.mEta-row.tEta} 
            self.var_d_0  ={'mPt_':row.mPt,'tPt_':row.tPt,'tMtToPfMet_type1_':row.tMtToPfMet_JetEnUp,'m_t_DPhi_':abs(row.m_t_DPhi),'tDPhiToPfMet_type1_':row.tDPhiToPfMet_JetEnUp,'type1_pfMetEt_':row.type1_pfMet_shiftedPt_JetEnUp,'deltaeta_m_t_':abs(row.mEta-row.tEta),'m_t_collinearmass_':row.m_t_collinearmass_JetEnUp}
            self.var_d_1  =self.var_d_0 
            self.var_d_2  =self.var_d_0
           # self.var_d_2  ={'mPt_':row.mPt,'tPt_':row.tPt,'mMtToPfMet_type1_':row.mMtToPfMet_JetEnUp,'tMtToPfMet_type1_':row.tMtToPfMet_JetEnUp,'m_t_DPhi_':row.m_t_DPhi,'tDPhiToPfMet_type1_':row.tDPhiToPfMet_JetEnUp,'type1_pfMetEt_':row.type1_pfMet_shiftedPt_JetEnUp,'jetVeto30_':row.jetVeto30_JetEnUp,'vbfMass_':row.vbfMass_JetEnUp/2600.0,'vbfDeta_':row.vbfDeta_JetEnUp/10.0,'deltaeta_m_t_':row.mEta-row.tEta} 
            #self.var_d_2  ={'mMtToPfMet_type1_':row.mMtToPfMet_JetEnUp,'tMtToPfMet_type1_':row.tMtToPfMet_JetEnUp,'m_t_DPhi_':row.m_t_DPhi,'tDPhiToPfMet_type1_':row.tDPhiToPfMet_JetEnUp,'type1_pfMetEt_':row.type1_pfMet_shiftedPt_JetEnUp,'jetVeto30_':row.jetVeto30_JetEnUp,'vbfMass_':row.vbfMass_JetEnUp,'vbfDeta_':row.vbfDeta_JetEnUp,'deltaeta_m_t_':row.mEta-row.tEta} 
#            if row.jetVeto30_JetEnUp==0:
...
#               MVA2=self.functor(**self.var_d_2)
            ...
            #        if MVA0>-0.01: 
                    ...
                    if RUN_OPTIMIZATION:
                       for  i in optimizer_new.compute_regions_0jet(row.tPt, row.mPt, deltaPhi(row.mPhi,row.tPhi),abs(row.tDPhiToPfMet_JetEnUp),row.tMtToPfMet_JetEnUp):
             ...
             #       if MVA0>-0.01:
                    ...
                    if RUN_OPTIMIZATION:
                       for  i in optimizer_new.compute_regions_1jet(row.tPt, row.mPt,deltaPhi(row.mPhi,row.tPhi),abs(row.tDPhiToPfMet_JetEnUp),row.tMtToPfMet_JetEnUp):
              ...
              #      if MVA0>-0.01:
                    ...
                    #if (row.vbfMass_JetEnUp>550 and abs(row.vbfDeta_JetEnUp)>3.5): 
                    if (row.vbfMass_JetEnUp>550): 
                          self.fill_histos(row,'vbf_vbfIsoSS')
                    if RUN_OPTIMIZATION:
                          for  i in optimizer_new.compute_regions_2jettight(row.tPt, row.mPt,row.tMtToPfMet_JetEnUp,row.vbfMass_JetEnUp,row.vbfDeta_JetEnUp):
                             tmp=os.path.join("vbf_vbfIsoSS",i)
                             self.fill_histos(row,tmp,False)
                    #if not (row.vbfMass_JetEnUp>550 and abs(row.vbfDeta_JetEnUp)>3.5):
                    if not (row.vbfMass_JetEnUp>550):
                             self.fill_histos(row,'vbf_ggIsoSS')
                    #      elif row.vbfMass_JetEnUp<100:
                    #         self.fill_histos(row,'boostIsoSS')
   
   
                    if RUN_OPTIMIZATION:
                       for  i in optimizer_new.compute_regions_2jetloose(row.tPt, row.mPt,row.tMtToPfMet_JetEnUp,row.vbfMass_JetEnUp,row.vbfDeta_JetEnUp):
...
#              print row.m_t_collinearmass_JetEnUp
             ...
             #     self.fill_histos(row,'gg',False)
              ...
              #if (row.jetVeto30_JetEnUp>=2 and row.vbfJetVeto30_JetEnUp <= 0) :
              if (row.jetVeto30_JetEnUp==2) :
#                  if RUN_OPTIMIZATION:
#                     for  i in optimizer.compute_regions_2jet(row.tPt, row.mPt,row.tMtToPfMet_JetEnUp,row.vbfMass_JetEnUp,row.vbfDeta_JetEnUp):
...
#                 self.fill_histos(row,'notIsoNotWeighted',False)
           ...
           #          self.fill_histos(row,'ggNotIso',True)
                 if row.jetVeto30_JetEnUp==1:
                  #  # if RUN_OPTIMIZATION:
                  #  #    for  i in optimizer.compute_regions_1jet(row.tPt, row.mPt,row.tMtToPfMet_JetEnUp):
         ...
         #            self.fill_histos(row,'boostNotIso',True)
            ...
            #     #          self.fill_histos(row,tmp,True)
                  ...
                  #   if MVA0>-0.01:
                        #if row.vbfMass_JetEnUp>=100 and row.vbfMass<550:
                     #if not (row.vbfMass_JetEnUp>550 and abs(row.vbfDeta_JetEnUp)>3.5):
                     if not (row.vbfMass_JetEnUp>550):
...
#                 self.fill_histos(row,'notIsoNotWeighted',False)
                 ...
                 if  row.jetVeto30_JetEnUp==0:
                    # #if RUN_OPTIMIZATION:
                       # for  i in optimizer.compute_regions_0jet(row.tPt, row.mPt, deltaPhi(row.mPhi,row.tPhi),row.tMtToPfMet_JetEnUp):
                    # #   for  i in optimizer.compute_regions_0jet(row.tPt, row.mPt, deltaPhi(row.mPhi,row.tPhi),abs(row.mDPhiToPfMet_JetEnUp),abs(row.tDPhiToPfMet_JetEnUp),row.tMtToPfMet_JetEnUp):
