...
class AnalyzeLFVMuTau_progress_TES3(MegaBase):
    tree = 'mt/final/Ntuple'
    #tree = 'New_Tree'

    def __init__(self, tree, outfile, **kwargs):
        super(AnalyzeLFVMuTau_progress_TES3, self).__init__(tree, outfile, **kwargs)
...
#           self.Z_reweight_H=self.Z_reweight.Get('zptmass_histo')
    def begin(self):
       ...
       #    name=[]
           sysneed=['MESUp','MESDown','TES0Up','TES0Down','TES1Up','TES1Down','TES10Up','TES10Down','UESUp','UESDown','UES0Up','UES0Down','UES1Up','UES1Down','UES10Up','UES10Down']
...
#                              self.fill_histos(row,'vbf_vbfNotIsoM2ndDown',True,"2ndDown",faketype="muonfake")
    #        global Sysin
            ...
            if self.Sysin and (not self.is_data) :
                 ...
                 sysneedTES.append('TES'+DMinstring+'Up')
                 sysneedTES.append('TES'+DMinstring+'Down')
              ...
              #   print sysneedTES
                 ...
                 sysneedUES=[]
                 DMinstring=str(int(round(row.tDecayMode))) 
                 sysneedUES.append('UES'+DMinstring+'Up')
                 sysneedUES.append('UES'+DMinstring+'Down')
                 for N in sysneedUES:
                     self.SetsysZero(row)
                     tmpname='self.DoUES'
                     if 'Up' in N:
                     ...
                     self.VariableCalculate(row) 
                    # if 'Up' in N:
                    #     print "Up DM the Tau Pt before %f and after correction %f and ratio %f"  %(row.type1_pfMet_shiftedPt_UnclusteredEnUp,self.tmpHula[0],row.type1_pfMet_shiftedPt_UnclusteredEnUp/self.tmpHula[0])
                    #    # print "UP SYS the Tau Pt before %f and after correction %f and ratio %f"  %(self.tau_Pt_C,self.tmpHula[0],self.tmpHula[0]/self.tau_Pt_C)
                    # if 'Down' in N:
                    #     print "Down DM the Tau Pt before %f and after correction %f and ratio %f"  %(row.type1_pfMet_shiftedPt_UnclusteredEnDown,self.tmpHula[0],row.type1_pfMet_shiftedPt_UnclusteredEnDown/self.tmpHula[0])
                    #     print "Up Down DM the Tau Pt before %f and after correction %f and ratio %f"  %(row.type1_pfMet_shiftedPt_UnclusteredEnUp,row.type1_pfMet_shiftedPt_UnclusteredEnDown,row.type1_pfMet_shiftedPt_UnclusteredEnUp/row.type1_pfMet_shiftedPt_UnclusteredEnDown)
                  #   if self.DoMES==1:
                  #       print " %s   %f" %(i,self.collMass_type1_new)
                     if self.obj2_iso(row) and self.oppositesign(row) and self.obj1_iso(row) and self.kinematics(row): 
                        for j in basechannelsI: 
