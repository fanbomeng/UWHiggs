...
class AnalyzeLFVMuTauPostBDT_progress_v15(MegaBase):
    tree = 'mt/final/Ntuple'
    #tree = 'New_Tree'

    def __init__(self, tree, outfile, **kwargs):
        super(AnalyzeLFVMuTauPostBDT_progress_v15, self).__init__(tree, outfile, **kwargs)
...
#      print "Sysin value in the correction %f" %self.Sysin
    ...
    def getFakeRateFactorFANBOPt(self,row, fakeset):
            ...
            #   if  row.jetVeto30==0:
            #       if abs(row.tEta)<1.479:
            #              fTauIso=0.212462-0.000559545*(self.tau_Pt_C-30)
            #       else:
            #              fTauIso=0.254039-0.000987035*(self.tau_Pt_C-30)
            #   elif  row.jetVeto30==1:
            #       if abs(row.tEta)<1.479:
            #              fTauIso=0.198585-0.00027885*(self.tau_Pt_C-30)
            #       else:
            #              fTauIso=0.217089-0.000750122*(self.tau_Pt_C-30)
            #   elif  row.jetVeto30==2:
            #       if row.vbfMass < 550:
            #          if abs(row.tEta)<1.479:
            #                 fTauIso=0.198316-0.00042984*(self.tau_Pt_C-30)
            #          else:
            #                 fTauIso=0.220919-0.000561159*(self.tau_Pt_C-30)
            #       else:
            #          if abs(row.tEta)<1.479:
            #                 if row.tPt<40:
            #                    fTauIso=0.1768173
            ...
            #                    fTauIso=0.2553783
            #   else:
            #      print "the number of Jets %f the tau Eta %f and tau Pt %f and vbfMass %f" %(row.jetVeto30,row.tEta,row.tPt,row.vbfMass) 
#           #       fTauIso=0
...
#                 print "comes in as USE111111111111111111"
    ...
    def WeightJetbin(self,row):
        ...
        if self.ls_Jets:
           ...
           if self.ls_Wjets:
              if row.numGenJets == 0:
                 return  1.0/(eval("weightNormal_MORE_Sold."+"WJetsToLNu_TuneCUETP8M1_13TeV_madgraphMLM_pythia8")) 
              else:
                 return 1.0/(eval("weightNormal_MORE_Sold."+"W"+str(int(row.numGenJets))+"JetsToLNu_TuneCUETP8M1_13TeV_madgraphMLM_pythia8"))
           if self.ls_ZTauTau:
              if row.numGenJets == 0:
                 return  1.0/(eval("weightNormal_MORE_Sold."+"ZTauTauJetsToLL_M_50_TuneCUETP8M1_13TeV_madgraphMLM_pythia8")) 
              else: 
                 return 1.0/(eval("weightNormal_MORE_Sold."+"ZTauTau"+str(int(row.numGenJets))+"JetsToLL_M_50_TuneCUETP8M1_13TeV_madgraphMLM_pythia8"))
           if self.ls_DY:
              if row.numGenJets == 0:
                 return  1.0/(eval("weightNormal_MORE_Sold."+"DYJetsToLL_M_50_TuneCUETP8M1_13TeV_madgraphMLM_pythia8")) 
              else: 
                 return 1.0/(eval("weightNormal_MORE_Sold."+"DY"+str(int(row.numGenJets))+"JetsToLL_M_50_TuneCUETP8M1_13TeV_madgraphMLM_pythia8"))
        else:
              return 1.0/(eval("weightNormal_MORE_Sold."+self.weighttarget))
