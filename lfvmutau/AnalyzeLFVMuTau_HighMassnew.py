...
class AnalyzeLFVMuTau_HighMassnew(MegaBase):
    tree = 'mt/final/Ntuple'

    def __init__(self, tree, outfile, **kwargs):
        super(AnalyzeLFVMuTau_HighMassnew, self).__init__(tree, outfile, **kwargs)
...
#            self.book(names[x], "singleIsoTkMu22Pass", "singleIsoTkMu22Pass", 12 ,-0.1,1.1)
           ...
           # self.book(names[x], "nvtx", "Number of vertices", 100, -0.5, 100.5)
            ...
            if not self.light: 
                  ...
                  self.book(names[x], "tGenPdgId", "tGenPdgId", 100,-50,50)
                  self.book(names[x], "abstGenPdgId", "abstGenPdgId", 50,0,50)
...
#            self.book(names[x], "vbfDphihjnomet","",100,0,4)
    ...
    def getFakeRateFactorFANBOPt(self,row, fakeset):
         ...
         #             fTauIso=0.1372
           ...
           #          if  row.tDecayMode==0:
           #              if self.tau_Pt_C<95: 
           #                 fTauIso=0.248922-0.00116575*(self.tau_Pt_C-30)
           #              else:
           #                 fTauIso=0.252998
           #          elif  row.tDecayMode==1:
           #              if self.tau_Pt_C<105:
           #                 fTauIso=0.236666-0.000963375*(self.tau_Pt_C-30)
           #              else:
           #                 fTauIso=0.202317
           #          elif  row.tDecayMode==10:
           #              if self.tau_Pt_C<95:
...
#parameter is shifted up, and the last 1 stand for this is in the EB region 
               elif self.DoFakeshapeDM==20170111:  # 0(DM)   1(first parameter) 1(up) 1(Low range) 
                      fTauIso=(0.248922+0.00726796)-0.00116575*(self.tau_Pt_C-30)
               elif self.DoFakeshapeDM==20170112:  # 0(DM)   1(first parameter) 1(up)  2(High range)
                      fTauIso=0.252998+0.0374291 
                      ...
                      fTauIso=(0.248922)-(0.00116575+0.000418365)*(self.tau_Pt_C-30)
             ...
             #         fTauIso=(0.270796)-(0.000821388-0.000836319)*(self.tau_Pt_C-30)
                      ...
                      fTauIso=(0.236666+0.00416335)-0.000963375*(self.tau_Pt_C-30)
               elif self.DoFakeshapeDM==20171112:  # 1(DM)   1(first parameter) 1(up)
                      fTauIso=0.202317+0.0191158
                      ...
                      fTauIso=(0.236666)-(0.000963375+0.00024676)*(self.tau_Pt_C-30)
...
#           histos[name+'/mMtToPfMet_type1'].Fill(row.mMtToPfMet_type1,weight)
           ...
           histos[name+'/tGenPdgId'].Fill(row.tGenPdgId,weight)
           histos[name+'/abstGenPdgId'].Fill(abs(row.tGenPdgId),weight)
...
# This function is to deal with the recombine with the number of Jets in the sample of W+jets and DY+jets
    ...
    def WjetsEnrich(self,row):
        ...
        if (self.tMtToPfMet_type1_new>80):
