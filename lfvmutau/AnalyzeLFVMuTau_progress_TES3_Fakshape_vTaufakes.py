...
class AnalyzeLFVMuTau_progress_TES3_Fakshape_vTaufakes(MegaBase):
    tree = 'mt/final/Ntuple'
    #tree = 'New_Tree'

    def __init__(self, tree, outfile, **kwargs):
        super(AnalyzeLFVMuTau_progress_TES3_Fakshape_vTaufakes, self).__init__(tree, outfile, **kwargs)
            ...
            if fakeset=="def":
               if  row.jetVeto30==0:
                   ...
                   if abs(row.tEta)<1.479:
                      ...
                      if abs(row.tEta)<1.479:
                             fTauIso=0.19528-0.000201737*(self.tau_Pt_C-30)
                      else:
                             fTauIso=0.21857-0.000373724*(self.tau_Pt_C-30)
               else:
