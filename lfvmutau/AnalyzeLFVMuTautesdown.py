...
#	metE = row.type1_pfMet_shiftedPt_TauEnDown
#	metPhi = row.type1_pfMet_shiftedPhi_TauEnDown
...
#        den=math.sqrt(xth)
#
#        mass=row.m_t_Mass_TauEnDown/den
#
...
class AnalyzeLFVMuTautesdown(MegaBase):
    tree = 'mt/final/Ntuple'
    #tree = 'New_Tree'

    def __init__(self, tree, outfile, **kwargs):
        super(AnalyzeLFVMuTautesdown, self).__init__(tree, outfile, **kwargs)
...
#           tmpMet=self.Metcorected.CorrectByMeanResolution(row.type1_pfMet_shiftedPt_TauEnDown*math.cos(row.type1_pfMet_shiftedPhi_TauEnDown),row.type1_pfMetEt*math.sin(row.type1_pfMetPhi),row.genpX,row.genpY,row.vispX,row.vispY,int(round(row.jetVeto30)))#,self.pfmetcorr_ex,self.pfmetcorr_ey)
...
#	histos[name+'/mDPhiToPfMet_tDPhiToPfMet'].Fill(abs(row.mDPhiToPfMet_TauEnDown),abs(row.tDPhiToPfMet_TauEnDown),weight)
#	histos[name+'/mDPhiToPfMet_ggdeltaphi'].Fill(abs(row.mDPhiToPfMet_TauEnDown),deltaPhi(row.mPhi, row.tPhi),weight)
#	histos[name+'/tDPhiToPfMet_ggdeltaphi'].Fill(abs(row.tDPhiToPfMet_TauEnDown),deltaPhi(row.mPhi, row.tPhi),weight)
#	histos[name+'/tDPhiToPfMet_tMtToPfMet_type1'].Fill(abs(row.tDPhiToPfMet_TauEnDown),row.tMtToPfMet_TauEnDown,weight)
...
#           return False
       if row.tPt < 30:  #was 35   #newcuts30
           return False
#       if row.tMtToPfMet_TauEnDown > 105:  #was 50   #newcuts65
#           return False
       if self.ls_recoilC and MetCorrection:
...
#	    return False
  ...
  #          return False
    ...
    def vbf_gg(self,row):
        ...
        if row.mPt < 25:   #was 40    #newcut 25
       		return False
   #     if row.tMtToPfMet_TauEnDown > 105: #was 35   #newcuts 55
   #             return False
        if self.ls_recoilC and MetCorrection:
            if self.tMtToPfMet_type1MetC > 105:  #was 50   #newcuts65
               return False
