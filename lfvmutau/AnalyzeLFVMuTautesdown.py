...
def getmtMass(row,sys='none'):
	...
	if(sys=='tesup'):
		return row.m_t_Mass_TauEnDown_TauEnUp
	if(sys=='tesdown'):
		return row.m_t_Mass_TauEnDown_TauEnDown

def gettMass(row,sys='none'):
        if(sys=='none' or 'jes' in sys or 'ues' in sys):
                return row.tMass_TauEnDown
        if(sys=='tesup'):
                return row.tMass_TauEnDown_TauEnUp
        if(sys=='tesdown'):
                return row.tMass_TauEnDown_TauEnDown
...
def getmtcollMass(row,sys='none'):
        if (sys=='none'):
                ...
                return row.m_t_collinearmass_TauEnDown_JetEnDown
        elif (sys=='jesup'):
                return row.m_t_collinearmass_TauEnDown_JetEnUp
        elif (sys=='uesdown'):
                return row.m_t_collinearmass_TauEnDown_UnclusteredEnDown
        elif (sys=='uesup'):
                return row.m_t_collinearmass_TauEnDown_UnclusteredEnUp
        elif (sys=='tesup'):
		return row.m_t_collinearmass_TauEnDown_TauEnUp
	elif (sys=='tesdown'):
		return row.m_t_collinearmass_TauEnDown_TauEnDown
...
#	    return False
	...
	if(row.vbfNJets<2):
	    return False
	if(abs(row.vbfDeta)<3.2 or (row.vbfMass < 240)):   #was 2.5    #newcut 2.0
	    return False
#        if row.vbfMass > 240:    #was 200   newcut 325
#	    return False
        if row.vbfJetVeto30 > 0:
...
#              print row.m_t_collinearmass_TauEnDown
             ...
             #     self.fill_histos(row,'gg',False)
              ...
              if  row.jetVeto30==0:
                  if RUN_OPTIMIZATION:
                     for  i in optimizer.compute_regions_0jet(row.tPt, row.mPt, deltaPhi(row.mPhi,row.tPhi),abs(row.tDPhiToPfMet_TauEnDown),row.tMtToPfMet_TauEnDown):
 ...
 #                              self.fill_histos(row,'ggTD10',False)
             ...
             #     self.fill_histos(row,'boost',False)
              if row.jetVeto30==1:
                  if RUN_OPTIMIZATION:
                     for  i in optimizer.compute_regions_1jet(row.tPt, row.mPt,deltaPhi(row.mPhi,row.tPhi),abs(row.tDPhiToPfMet_TauEnDown),row.tMtToPfMet_TauEnDown):
