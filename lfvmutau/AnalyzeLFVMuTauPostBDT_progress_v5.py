...
class AnalyzeLFVMuTauPostBDT_progress_v5(MegaBase):
    tree = 'mt/final/Ntuple'
    #tree = 'New_Tree'

    def __init__(self, tree, outfile, **kwargs):
        super(AnalyzeLFVMuTauPostBDT_progress_v5, self).__init__(tree, outfile, **kwargs)
...
#           self.Z_reweight_H=self.Z_reweight.Get('zptmass_histo')
    def begin(self):
       ...
       #    name=[]
        ...
        #           names.append(j+i)                    
           JESsys=["_CMS_JetAbsoluteFlavMapDown","_CMS_JetAbsoluteFlavMapUp","_CMS_JetSinglePionHCALUp","_CMS_JetTimePtEtaDown","_CMS_JetTimePtEtaUp"]
           for i in  JESsys:
               for j in basechannels:
                   names.append(j+i)
...
#           print "metlorenz %f %f %f" %(metlorenz.Pt(),metlorenz.Px(),metlorenz.Py())
    ...
    def VariableCalculate(self,row,tau_Pt_C,MET_tPtC):
           ...
           if self.DoUES==2:
              if not self.ls_DY:
                 ...
                 MVA0fill_new=self.functor(**var_d_0)
                 var_d_0_2  ={'mPt_':row.mPt,'tPt_':tau_Pt_C,'tMtToPfMet_type1_':tMtToPfMet_type1_new,'m_t_DPhi_':abs(row.m_t_DPhi),'tDPhiToPfMet_type1_':abs(row.tDPhiToPfMet_type1),'type1_pfMetEt_':tmpHulaUESDown[0],'deltaeta_m_t_':abs(row.mEta-row.tEta),'m_t_collinearmass_':collMass_type1_new}
                 MVA0fill_new=self.functor(**var_d_0)
                 MVA0fill_new_1=self.functor(**var_d_0_2)
                 if row.jetVeto30==0:
                    print 'THE BDT OUT PUT THOUGHT %f and old Phi cuts %f' %(MVA0fill_new,MVA0fill_new_1)
    ...
    def VariableCalculateJES(self,row,JESShiftedMet,JESShiftedPhi,tau_Pt_C): 
           ...
           if not self.ls_DY:
              ...
              tMtToPfMet_type1_new=transverseMass_v2(taulorenz,metlorenz)
              mMtToPfMet_type1_new=transverseMass_v2(muonlorenz,metlorenz)
              tmptDphiToPfMet=abs(row.tPhi-JESShiftedPhi)
              if tmptDphiToPfMet>3.1415926:
                 tmptDphiToPfMet=2*3.1415926-tmptDphiToPfMet
              ...
              var_d_0  ={'mPt_':row.mPt,'tPt_':tau_Pt_C,'tMtToPfMet_type1_':tMtToPfMet_type1_new,'m_t_DPhi_':abs(row.m_t_DPhi),'tDPhiToPfMet_type1_':tmptDphiToPfMet,'type1_pfMetEt_':metlorenz.Pt(),'deltaeta_m_t_':abs(row.mEta-row.tEta),'m_t_collinearmass_':collMass_type1_new}
              var_d_0_1 ={'mPt_':row.mPt,'tPt_':tau_Pt_C,'tMtToPfMet_type1_':tMtToPfMet_type1_new,'m_t_DPhi_':abs(row.m_t_DPhi),'tDPhiToPfMet_type1_':abs(row.tDPhiToPfMet_type1),'type1_pfMetEt_':metlorenz.Pt(),'deltaeta_m_t_':abs(row.mEta-row.tEta),'m_t_collinearmass_':collMass_type1_new}
              MVA0fill_new=self.functor(**var_d_0)
              MVA0fill_new_1=self.functor(**var_d_0_1)
              print 'tDphiMet %f BDToutput %f'  %(tmptDphiToPfMet,MVA0fill_new)
              print 'tDphiMet %f BDToutput %f   old'  %(abs(row.tDPhiToPfMet_type1),MVA0fill_new_1)
...
#            print 'the def tPt %f  and M_coll value %f'  %(self.tau_Pt_C,self.collMass_type1_new)   
            ...
            if self.Sysin:
                 ...
                 baseJESsys=["_CMS_JetAbsoluteFlavMapDown","_CMS_JetAbsoluteFlavMapUp","_CMS_JetTimePtEtaDown","_CMS_JetTimePtEtaUp"]
