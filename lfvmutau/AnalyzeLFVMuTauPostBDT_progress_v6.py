...
class AnalyzeLFVMuTauPostBDT_progress_v6(MegaBase):
    tree = 'mt/final/Ntuple'
    #tree = 'New_Tree'

    def __init__(self, tree, outfile, **kwargs):
        super(AnalyzeLFVMuTauPostBDT_progress_v6, self).__init__(tree, outfile, **kwargs)
...
#           print "metlorenz %f %f %f" %(metlorenz.Pt(),metlorenz.Px(),metlorenz.Py())
    ...
    def VariableCalculateJES(self,row,JESShiftedMet,JESShiftedPhi,tau_Pt_C): 
           ...
           if not self.ls_DY:
              ...
              var_d_0  ={'mPt_':row.mPt,'tPt_':tau_Pt_C,'tMtToPfMet_type1_':self.tMtToPfMet_type1_new,'m_t_DPhi_':abs(row.m_t_DPhi),'tDPhiToPfMet_type1_':abs(row.tPhi-JESShiftedPhi),'type1_pfMetEt_':metlorenz.Pt(),'deltaeta_m_t_':abs(row.mEta-row.tEta),'m_t_collinearmass_':collMass_type1_new}
           else:
