...
class AnalyzeLFVMuTau_progressChecks(MegaBase):
    tree = 'mt/final/Ntuple'
    #tree = 'New_Tree'

    def __init__(self, tree, outfile, **kwargs):
        super(AnalyzeLFVMuTau_progressChecks, self).__init__(tree, outfile, **kwargs)
...
#            met = math.sqrt(metpx*metpx+metpy*metpy)
    ...
    def getFakeRateFactorFANBOPt(self,row, fakeset):
         ...
         if fakeset=="def":
            if row.tDecayMode==5:
               print "fake rate enter and the Decaymode %f"  %row.tDecayMode
            if  row.tDecayMode==0:
         ...
         if fakeset=="1stDown":
            ...
            if  row.tDecayMode==10:
                fTauIso=0.186646-0.00416
         if row.tDecayMode==5:
            print "the fakeTauIso is %f" %fTauIso
         fakeRateFactor = fTauIso/(1.0-fTauIso)
         return fakeRateFactor
...
#                   continue
            ...
            if row.tDecayMode==5 or row.tDecayMode==6:
               print "the Tau decay mold is not normal"
            if self.ls_recoilC and MetCorrection: 
