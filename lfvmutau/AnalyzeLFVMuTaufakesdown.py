...
def getFakeRateFactor(row, isoName):
  if (isoName == "old"):
    if (row.tEta < 1.5):
      if (row.tDecayMode==0):
        fTauIso =0.2089 - 0.00113*(row.tPt-30)-0.045273 
      elif (row.tDecayMode==1):
        fTauIso = 0.2089 - 0.00113*(row.tPt-30)-0.0229301
      elif (row.tDecayMode==10):
        fTauIso = 0.2089 - 0.00113*(row.tPt-30)-0.0321169
    if (row.tEta >= 1.5):
      if (row.tDecayMode==0):
        fTauIso = 0.2089 - 0.00113*(row.tPt-30)-0.0602681
      elif (row.tDecayMode==1):
        fTauIso = 0.2089 - 0.00113*(row.tPt-30)-0.0382062
      elif (row.tDecayMode==10):
        fTauIso = 0.2089 - 0.00113*(row.tPt-30)-0.0427807
...
class AnalyzeLFVMuTaufakesdown(MegaBase):
    tree = 'mt/final/Ntuple'
    #tree = 'New_Tree'

    def __init__(self, tree, outfile, **kwargs):
        super(AnalyzeLFVMuTaufakesdown, self).__init__(tree, outfile, **kwargs)
...
#              print row.m_t_collinearmass
             ...
             #     self.fill_histos(row,'gg',False)
              ...
              if  row.jetVeto30==0:
                  if RUN_OPTIMIZATION:
                     for  i in optimizer.compute_regions_0jet(row.tPt, row.mPt, deltaPhi(row.mPhi,row.tPhi),abs(row.tDPhiToPfMet_type1),row.tMtToPfMet_type1):
 ...
 #                              self.fill_histos(row,'ggTD10',False)
             ...
             #     self.fill_histos(row,'boost',False)
              if row.jetVeto30==1:
                  if RUN_OPTIMIZATION:
                     for  i in optimizer.compute_regions_1jet(row.tPt, row.mPt,deltaPhi(row.mPhi,row.tPhi),abs(row.tDPhiToPfMet_type1),row.tMtToPfMet_type1):
