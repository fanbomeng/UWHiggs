...
class AnalyzeLFVMuTaubeforenewTuning(MegaBase):
    tree = 'mt/final/Ntuple'
    #tree = 'New_Tree'

    def __init__(self, tree, outfile, **kwargs):
        super(AnalyzeLFVMuTaubeforenewTuning, self).__init__(tree, outfile, **kwargs)
...
#                return False
	...
	if(abs(row.vbfDeta)>3.2):   #was 2.5    #newcut 2.0
	    return False
        if row.vbfMass > 700:    #was 20   newcut 240
	    return False
        if row.vbfJetVeto30 > 0:
            return False
        if row.bjetCISVVeto30Medium:
	...
	if(row.vbfNJets<2):
	    return False
	if(abs(row.vbfDeta)<3.2 or (row.vbfMass < 700)):   #was 2.5    #newcut 2.0
	    return False
