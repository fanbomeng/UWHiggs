...
class AnalyzeLFVMuTau_progressVIS(MegaBase):
    tree = 'mt/final/Ntuple'
   # self.tree = 'New_Tree'

    def __init__(self, tree, outfile, **kwargs):
        super(AnalyzeLFVMuTau_progressVIS, self).__init__(tree, outfile, **kwargs)
...
#           names=names+sysname
	...
	for x in range(0,namesize):
            ...
            self.book(names[x],"m_t_Mass","m_t_Mass",300,0,300);
...
#            met = math.sqrt(metpx*metpx+metpy*metpy)
            ...
            #print mass
            return mass/den
    def m_t_Mass_newC(self,row,muonlorenz,taulorenz):
            mass=(muonlorenz+taulorenz).M()
            return mass
...
#           print "metlorenz %f %f %f" %(metlorenz.Pt(),metlorenz.Px(),metlorenz.Py())
           m_t_Mass_new=self.m_t_Mass_newC(row,muonlorenz,taulorenz)
           ...
           return(m_t_Mass_new,tMtToPfMet_type1_new)
    def VariableCalculate(self,row,tau_Pt_C,MET_tPtC):
           ...
           #print 'at line 871 the MES %f ' %self.DoMES
           if self.DoTES==1:
              tmpHula=Systematics.TESup(tau_Pt_C,MET_tPtC,row.tPhi,row.type1_pfMetPhi)
              taulorenz.SetPtEtaPhiM(tmpHula[0],row.tEta,row.tPhi,row.tMass) 
              muonlorenz.SetPtEtaPhiM(row.mPt,row.mEta,row.mPhi,row.mMass) 
              metlorenz.SetPtEtaPhiM(tmpHula[3],0,row.type1_pfMetPhi,0)
              m_t_Mass_new=self.m_t_Mass_newC(row,muonlorenz,taulorenz)
              tMtToPfMet_type1_new=transverseMass_v2(taulorenz,metlorenz)
           if self.DoTES==2:
              tmpHula=Systematics.TESdown(tau_Pt_C,MET_tPtC,row.tPhi,row.type1_pfMetPhi)
              taulorenz.SetPtEtaPhiM(tmpHula[0],row.tEta,row.tPhi,row.tMass) 
              muonlorenz.SetPtEtaPhiM(row.mPt,row.mEta,row.mPhi,row.mMass) 
              metlorenz.SetPtEtaPhiM(tmpHula[3],0,row.type1_pfMetPhi,0)
              m_t_Mass_new=self.m_t_Mass_newC(row,muonlorenz,taulorenz)
              tMtToPfMet_type1_new=transverseMass_v2(taulorenz,metlorenz)
           if self.DoMES==1:
              tmpHula=Systematics.MESup(row.mPt,row.mEta,MET_tPtC,row.mPhi,row.type1_pfMetPhi)
              taulorenz.SetPtEtaPhiM(self.tau_Pt_C,row.tEta,row.tPhi,row.tMass) 
              muonlorenz.SetPtEtaPhiM(tmpHula[0],row.mEta,row.mPhi,row.mMass) 
              metlorenz.SetPtEtaPhiM(tmpHula[3],0,row.type1_pfMetPhi,0)
              m_t_Mass_new=self.m_t_Mass_newC(row,muonlorenz,taulorenz)
              tMtToPfMet_type1_new=transverseMass_v2(taulorenz,metlorenz)
           if self.DoMES==2:
              tmpHula=Systematics.MESdown(row.mPt,row.mEta,MET_tPtC,row.mPhi,row.type1_pfMetPhi)
           ...
           if self.DoUES==1:
              ...
              tmpHula=Systematics.UESup(UES_shiftedMetup,row.type1_pfMet_shiftedPhi_UnclusteredEnUp)
              taulorenz.SetPtEtaPhiM(tau_Pt_C,row.tEta,row.tPhi,row.tMass) 
              muonlorenz.SetPtEtaPhiM(row.mPt,row.mEta,row.mPhi,row.mMass) 
              metlorenz.SetPtEtaPhiM(tmpHula[0],0,tmpHula[3],0)
              m_t_Mass_new=self.m_t_Mass_newC(row,muonlorenz,taulorenz)
              tMtToPfMet_type1_new=transverseMass_v2(taulorenz,metlorenz)
           if self.DoUES==2:
              if  row.tDecayMode==0:
              ...
              tmpHula=Systematics.UESdown(UES_shiftedMetdown,row.type1_pfMet_shiftedPhi_UnclusteredEnDown)
              taulorenz.SetPtEtaPhiM(tau_Pt_C,row.tEta,row.tPhi,row.tMass) 
              muonlorenz.SetPtEtaPhiM(row.mPt,row.mEta,row.mPhi,row.mMass) 
              metlorenz.SetPtEtaPhiM(tmpHula[0],0,tmpHula[3],0)
              m_t_Mass_new=self.m_t_Mass_newC(row,muonlorenz,taulorenz)
              tMtToPfMet_type1_new=transverseMass_v2(taulorenz,metlorenz)
           return(m_t_Mass_new,tMtToPfMet_type1_new,tmpHula[0])
...
#            global Sysin
            ...
            self.m_t_Mass_new,self.tMtToPfMet_type1_new=self.VariableCalculateTaucorrection(row,self.tau_Pt_C,self.MET_tPtC)
            ...
            if self.Sysin and (not self.is_data) :
                 ...
                 for i in sysneedI:
                     ...
                     if 'Down' in i:
                        ...
                        exec("%s = %d" % (tmpname,2))                   
                     self.m_t_Mass_new,self.tMtToPfMet_type1_new,self.MorTPtshifted=self.VariableCalculate(row,self.tau_Pt_C,self.MET_tPtC) 
                  #   if self.DoMES==1:
                 ...
                 for k in baseJESsys:
                     ...
                     self.m_t_Mass_new,self.tMtToPfMet_type1_new=self.VariableCalculateJES(row,eval(tmpname_5),eval(tmpname_6),self.tau_Pt_C)
