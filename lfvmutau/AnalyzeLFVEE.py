...
#  print 'TriggerEffi  %f' %TriggerEfficency
  #print "pu"
  #print str(pu)
  #return pu*e1reconcorr*e2reconcorr*e1iso*e2iso*TriggerEfficency
...
class AnalyzeLFVEE(MegaBase):
    tree = 'eet/final/Ntuple'
    #tree = 'New_Tree'

    def __init__(self, tree, outfile, **kwargs):
        super(AnalyzeLFVEE, self).__init__(tree, outfile, **kwargs)
        ...
        self.is_data = target.startswith('data_SingleElectron')
...
#	   weight = row.GenWeight * self.correction(row) #apply gen and pu reweighting to MC
   ...
   #        weight=weight*wtzpt
           #print 'the weight value     %f'    %wtzpt
...
#                continue
        ...
        #           continue
            ...
            if self.obj2_iso(row) and self.oppositesign(row):
                ...
                #print 'the data trigger value   %d'   %row.singleE25eta2p1TightPass
                if  self.tauinEB(row):
