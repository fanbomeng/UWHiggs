...
muon_pog_TriggerIso50_2016B= MuonPOGCorrections.make_muon_pog_Mu50orTkMu50_2016ReRecoeffi()
#muon_pog_TriggerIso50_2016B= MuonPOGCorrections.make_muon_pog_Mu50orTkMu50_2016ReRecoSF()
...
class AnalyzeLFVMuTau_HighMassnewtest(MegaBase):
    tree = 'mt/final/Ntuple'

    def __init__(self, tree, outfile, **kwargs):
        super(AnalyzeLFVMuTau_HighMassnewtest, self).__init__(tree, outfile, **kwargs)
        ...
        self.is_Mass450=(bool('M450' in target))
        ...
        if not self.light :
           ...
           for i in self.highMassbase:
               for j in self.highMass:
                   names.append(i+j)
        if self.Sysin:
           sysneed_Fake=['TauFakeRate_p0_dm01_L_13TeVUp','TauFakeRate_p0_dm01_L_13TeVDown','TauFakeRate_p1_dm01_L_13TeVUp','TauFakeRate_p1_dm01_L_13TeVDown','TauFakeRate_p0_dm01_M_13TeVUp','TauFakeRate_p0_dm01_M_13TeVDown','TauFakeRate_p0_dm01_H_13TeVUp','TauFakeRate_p0_dm01_H_13TeVDown','TauFakeRate_p0_dm10_L_13TeVUp','TauFakeRate_p0_dm10_L_13TeVDown','TauFakeRate_p0_dm10_M_13TeVUp','TauFakeRate_p0_dm10_M_13TeVDown','TauFakeRate_p0_dm10_H_13TeVUp','TauFakeRate_p0_dm10_H_13TeVDown']
           #basechannels_Fake=['ggNotIso','ggNotIsoM','ggNotIsoMT','boostNotIso','boostNotIsoM','boostNotIsoMT']
           basechannels_Fake=['ggNotIso','boostNotIso']
...
#            self.book(names[x], "singleIsoTkMu22Pass", "singleIsoTkMu22Pass", 12 ,-0.1,1.1)
           ...
           # self.book(names[x], "nvtx", "Number of vertices", 100, -0.5, 100.5)
            ...
            if not self.light: 
                  ...
                  self.book(names[x], "tJetPartonFlavour", "tJetPartonFlavour", 100,-50,50)
                  self.book(names[x], "abstJetPartonFlavour", "abstJetPartonFlavour", 50,0,50)
                  self.book(names[x], "tGenMotherPdgId", "tGenPdgId", 100,-50,50)
                  self.book(names[x], "abstGenMotherPdgId", "abstGenPdgId", 50,0,50) 
                  ...
                  self.book2D(names[x],"mMtToPfMet_type1_tMtToPfMet_type1", "mMtToPfMet_type1_tMtToPfMet_type1",1000,0,1000,1000,0,1000)
...
#            self.book(names[x], "vbfDphihjnomet","",100,0,4)
    ...
    def mc_corrector_2016(self,row):
      ...
      if (not self.Sysin) or (self.DoTES) or self.DoUES or self.DoJES or self.DoFakeshapeDM or self.DoMFT or self.DoUESsp:
         ...
         if row.mPt<=199:
            ...
            m1id =muon_pog_PFTight1D_2016B(row.mPt*1.002)
            m1iso =muon_pog_TightIso1D_2016B(row.mPt*1.002)
         #   m1id =muon_pog_PFTight_2016B(row.mPt*1.002,abs(row.mEta))
         ...
         if row.mPt<=199:
            ...
            m1id =muon_pog_PFTight1D_2016B(row.mPt*0.998)
            m1iso =muon_pog_TightIso1D_2016B(row.mPt*0.998)
    ...
    def TauPtrangejudge(self,row):
        if row.tDecayMode==0 or row.tDecayMode==1:
           if self.tau_Pt_C<90:
              return 1
           elif self.tau_Pt_C<=220:
              return 2
           else:
              return 3
    ...
    def getFakeRateFactorFANBOPt(self,row, fakeset):
         ...
         #             fTauIso=0.1372
            ...
            if fakeset=="def":
                     if  row.tDecayMode==0 or  row.tDecayMode==1:
                         if self.tau_Pt_C<90: 
                            fTauIso=0.238265-0.000874124*(self.tau_Pt_C-30)
                         elif self.tau_Pt_C<220:
                            fTauIso=0.1967758
                         else:
                            fTauIso=0.129336
...
#the second 1 stands for the first parameter(P0) in the fake rate ration which is the form as P0+P1(x-30), the 3rd 1 stand for the if the parameter is up or down  and the last 1 stand for this is in which pt range
                      ...
                      fTauIso=(0.238265-0.00375437)-0.000874124*(self.tau_Pt_C-30)
               elif self.DoFakeshapeDM==201701211:  # 0&1(DM)   1(first parameter) 1(up) range1 
                      fTauIso=(0.238265)-(0.000874124+0.000223027)*(self.tau_Pt_C-30)
               elif self.DoFakeshapeDM==201701221:  # 0&1(DM)   1(first parameter) 1(down) range1 
                      fTauIso=(0.238265)-(0.000874124-0.000223027)*(self.tau_Pt_C-30)

               elif self.DoFakeshapeDM==201701112:  # 0&1(DM)   1(first parameter) 1(up) range2 
                      fTauIso=0.1967758+0.012903
               elif self.DoFakeshapeDM==201701122:  # 0&1(DM)   1(first parameter) 1(up) range2 
                      fTauIso=0.1967758-0.012903

               elif self.DoFakeshapeDM==201701113:  # 0&1(DM)   1(first parameter) 1(up) range3 
                      fTauIso=0.1293359+0.060556
               elif self.DoFakeshapeDM==201701123:  # 0&1(DM)   1(first parameter) 1(up) range3 
                      fTauIso=0.1293359-0.060556

               elif self.DoFakeshapeDM==201710111:  # 10(DM)   1(first parameter) 1(up) range1 
                      fTauIso=0.180779+0.002858
               elif self.DoFakeshapeDM==201710121:  # 10(DM)   1(first parameter) 1(up) range1 
                      fTauIso=0.180779-0.002858

               elif self.DoFakeshapeDM==201710112:  # 10(DM)   1(first parameter) 1(up) range2 
                      fTauIso=0.123934+0.01674
               elif self.DoFakeshapeDM==201710122:  # 10(DM)   1(first parameter) 1(up) range2 
                      fTauIso=0.123934-0.01674

               elif self.DoFakeshapeDM==201710113:  # 10(DM)   1(first parameter) 1(up) range2 
                      fTauIso=0.083955+0.080383
               elif self.DoFakeshapeDM==201710123:  # 10(DM)   1(first parameter) 1(up) range2 
                      fTauIso=0.083955-0.080383
...
#                      fTauIso=0.214062-(0.000896065-0.000395512)*(self.tau_Pt_C-30)
            ...
            #print 'the fake factor is %f and the fTauIso  %f'  %(fakeRateFactor,fTauIso)
...
#show an apparently effect
  ...
  # 	   weight =row.GenWeight * self.correction(row)*self.WeightJetbin(row)
        ...
        if self.ls_DY or self.ls_ZTauTau:
           ...
           weight=weight*wtzpt
#        if 'EnWjets' in name and (not(self.is_data)):
#           weight=weight*bTagSFrereco.bTagEventWeight(row.bjetCISVVeto30Medium,row.jb1pt,row.jb1hadronflavor,row.jb2pt,row.jb2hadronflavor,1,btagSys,0)
...
#           histos[name+'/mMtToPfMet_type1'].Fill(row.mMtToPfMet_type1,weight)
           ...
           histos[name+'/abstJetPartonFlavour'].Fill(abs(row.tJetPartonFlavour),weight)
           histos[name+'/tGenMotherPdgId'].Fill(row.tGenMotherPdgId,weight)
           histos[name+'/abstGenMotherPdgId'].Fill(abs(row.tGenMotherPdgId),weight)
...
# This function is also used in the JES shape systermatics variable calculation, the upward part of it is to recalculate the Jet energy scale systermatics after the Tau energy scale correction. We need to do it is because JES is calculated in the Ntuple steps, and TES correction required from Tau POG is a global correctoin which should be applied to all of the real Tau
    ...
    def presel(self, row):
        ...
        if not (row.singleMu50Pass or row.singleTkMu50Pass):
            return   False
        return True 
...
# This function is to deal with the recombine with the number of Jets in the sample of W+jets and DY+jets
    def WeightJetbin(self,row):
        ...
        if self.ls_Jets:
           ...
           if self.ls_Wjets:
              if row.numGenJets == 0:
                 return  1.0/(eval("weightNormalNewtrigger."+"WJetsToLNu_TuneCUETP8M1_13TeV_madgraphMLM_pythia8")) 
              else:
                 return 1.0/(eval("weightNormalNewtrigger."+"W"+str(int(row.numGenJets))+"JetsToLNu_TuneCUETP8M1_13TeV_madgraphMLM_pythia8"))
           if self.ls_ZTauTau:
              if row.numGenJets == 0:
                 return  1.0/(eval("weightNormalNewtrigger."+"ZTauTauJetsToLL_M_50_TuneCUETP8M1_13TeV_madgraphMLM_pythia8")) 
              else: 
                 return 1.0/(eval("weightNormalNewtrigger."+"ZTauTau"+str(int(row.numGenJets))+"JetsToLL_M_50_TuneCUETP8M1_13TeV_madgraphMLM_pythia8"))
           if self.ls_DY:
              if row.numGenJets == 0:
                 return  1.0/(eval("weightNormalNewtrigger."+"DYJetsToLL_M_50_TuneCUETP8M1_13TeV_madgraphMLM_pythia8")) 
              else: 
                 return 1.0/(eval("weightNormalNewtrigger."+"DY"+str(int(row.numGenJets))+"JetsToLL_M_50_TuneCUETP8M1_13TeV_madgraphMLM_pythia8"))
        else:
...
#            if self.is_data  and (not row.bjetCISVVeto30Medium):
               return True
 #           if not(self.is_data):
 #              return True
        return False
    ...
    def gg(self,row,MassPoint='def'):
        if not RUN_OPTIMIZATION_v2:
           ...
           if  MassPoint=='200' or  MassPoint=='300' or MassPoint==200:
               if muontmpPt< 60 or tautmpPt<30 or self.tMtToPfMet_type1_new >105:   #was45     #newcuts 25 
                return False 
           else  :
             if muontmpPt <150 or tautmpPt<45  or self.tMtToPfMet_type1_new >200:   #was45     #newcuts 25 
                return False 
   #     elif  MassPoint=='600':
...
#           return False
 ...
 #               return False 
             ...
             if muontmpPt< 150 or tautmpPt<45 or self.tMtToPfMet_type1_new >230:   #was45     #newcuts 25 
                  return False 
 #       elif  MassPoint=='600':
...
#        return bool( (not row.tByTightIsolationMVArun2v1DBoldDMwLT) and  row.tByLooseIsolationMVArun2v1DBoldDMwLT)
    ...
    def process(self):
        ...
        for row in self.tree:
            ...
            if (not self.presel(row)) and self.is_data:
                  ...
                  continue
            #if row.mPt<24:
            #   print "the Pt value is %f"  %row.mPt
                  ...
                  if (not self.light_v2):
                       ...
                       if (not self.light):
                           ...
                           for massname in self.highMass:
                              if self.gg(row,massname):
                                 self.fill_histos(row,'selectionSS'+massname,False)	
                           for massname in self.highMass:
                              if self.gg(row,massname):
                                 self.fill_histos(row,'selectionggSS'+massname,False)	
                       if RUN_OPTIMIZATION_v2:
                     ...
                     if row.jetVeto30==1:
                       if (not self.light):
                           ...
                           for massname in self.highMass:
                              if self.boost(row,massname):
                                 self.fill_histos(row,'selectionboostSS'+massname,False)	
                       if RUN_OPTIMIZATION_v2:
               ...
               if not self.obj2_iso(row) and not self.oppositesign(row) :
                      ...
                      if (not self.light):
                         if row.jetVeto30==0:
                           ...
                           for massname in self.highMass:
                              if self.gg(row,massname):
                                 self.fill_histos(row,'selectionnotIsoSS'+massname,True)	
                         if row.jetVeto30==1:
                 ...
                 if not self.light:
                    ...
                    #      print "tGenMother PDG iD %f" %row.tGenMotherPdgId
                       ...
                       if self.WjetsEnrich2(row):
                          self.fill_histos(row,'preslectionEnWjets2')
                       if self.WjetsEnrich3(row):
                          self.fill_histos(row,'preslectionEnWjets3')
              ...
              if self.obj1_iso(row):
                 ...
                 if (not self.light):
                    if  fakeset:
                       ...
                       if self.WjetsEnrich2(row):
                          self.fill_histos(row,'notIsoEnWjets2',True)
                       if self.WjetsEnrich3(row):
                          self.fill_histos(row,'notIsoEnWjets3',True)
                       if self.ZttEnrich(row,self.m_t_Mass_new,self.tMtToPfMet_type1_new,self.mMtToPfMet_type1_new):
...
# starting from here, is to fill in the systermatic shape related histotrams
            ...
            if self.Sysin:
                 ...
                 if DMinstring=='0' or DMinstring=='1':
                    DMinstring='01'
                    
                 taurangevalue=self.TauPtrangejudge(row)
                 for M in sysneedFAKES:
                     ...
                     if 'Up' in M and (M.split('dm',1)[1]).split('_',2)[0]==DMinstring:
                        if ('L' in M) and (taurangevalue==1):
                           valuehere='2017'+DMinstring+Para+'1'+'1'
                           exec("%s = %d" % (tmpname,int(valuehere)))                   
                        if ('M' in M) and (taurangevalue==2):
                           valuehere='2017'+DMinstring+Para+'1'+'2'
                           exec("%s = %d" % (tmpname,int(valuehere)))                   
                        elif 'H' in M and (taurangevalue==3):
                           valuehere='2017'+DMinstring+Para+'1'+'3'
                           exec("%s = %d" % (tmpname,int(valuehere)))       
                        else:
                        ...
                        if 'L' in M and (taurangevalue==1):
                           ...
                           exec("%s = %d" % (tmpname,int(valuehere)))                   
                        elif 'M' in M and (taurangevalue==2):
                           valuehere='2017'+DMinstring+Para+'2'+'2'
                           exec("%s = %d" % (tmpname,int(valuehere)))                  
                        elif 'H' in M and (taurangevalue==3):
                           valuehere='2017'+DMinstring+Para+'2'+'3'
                           exec("%s = %d" % (tmpname,int(valuehere)))                  
                        else:
                           exec("%s = %d" % (tmpname,20170000))                   
                     else:
