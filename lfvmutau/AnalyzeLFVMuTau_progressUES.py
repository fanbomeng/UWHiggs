...
class AnalyzeLFVMuTau_progressUES(MegaBase):
    tree = 'mt/final/Ntuple'
    #tree = 'New_Tree'

    def __init__(self, tree, outfile, **kwargs):
        super(AnalyzeLFVMuTau_progressUES, self).__init__(tree, outfile, **kwargs)
...
#           self.Z_reweight_H=self.Z_reweight.Get('zptmass_histo')
    def begin(self):
       ...
       #    name=[]
           #sysneed=['MESUp','MESDown','TESUp','TESDown','UESUp','UESDown','UESUpCheck','UESDownCheck']
           sysneed=['UESUpCheck','UESDownCheck']
...
#            met = math.sqrt(metpx*metpx+metpy*metpy)
    ...
    def getFakeRateFactorFANBOPt(self,row, fakeset):
         ...
         if fakeset=="def":
            if  row.tDecayMode==0:
                fTauIso=0.23879-0.0007289*(row.tPt-30)
            if  row.tDecayMode==1:
                fTauIso=0.235664-0.00076628*(row.tPt-30)
            if  row.tDecayMode==10:
                fTauIso=0.188160-0.000534633*(row.Pt-30)
         if self.DoTES==1:
            if  row.tDecayMode==0:
                fTauIso=0.23879-0.0007289*(row.tPt*1.03-30)
            if  row.tDecayMode==1:
                fTauIso=0.235664-0.00076628*(row.tPt*1.03-30)
            if  row.tDecayMode==10:
                fTauIso=0.188160-0.000534633*(row.Pt*1.03-30)
         if self.DoTES==2:
            if  row.tDecayMode==0:
                fTauIso=0.23879-0.0007289*(row.tPt*0.97-30)
            if  row.tDecayMode==1:
                fTauIso=0.235664-0.00076628*(row.tPt*0.97-30)
            if  row.tDecayMode==10:
                fTauIso=0.188160-0.000534633*(row.Pt*0.97-30)
         #if fakeset=="def":
         #   if  row.tDecayMode==0:
         #       fTauIso=0.229836
         #   if  row.tDecayMode==1:
    ...
    def getFakeRateFactormuon(self,row, fakeset):   #Ptbined
        ...
        if self.DoMES==1:
           if row.mPt*1.002<=30:
              ...
              fTauIso=0.851434
           elif row.mPt*1.002<=75:
              fTauIso=0.87976
           elif row.mPt*1.002<=500:
              fTauIso=0.95603
        if self.DoMES==2:
           if row.mPt*0.998<=30:
              fTauIso=0.80751
              ...
              fTauIso=0.851434
           elif row.mPt*0.998<=75:
              fTauIso=0.87976
           elif row.mPt*0.998<=500:
              fTauIso=0.95603
        if fakeset=="1stUp":
           if row.mPt<55:
 ...
 #       histos[name+'/vbfNJets30'].Fill(row.vbfNJets30, weight)
    ...
    def VariableCalculate(self,row):
           ...
           if self.DoUES==2:
              ...
              self.tMtToPfMet_type1_new=transverseMass_v2(taulorenz,metlorenz)
           if self.DoUES==3:
              self.tmpHula=Systematics.UESup(row.type1_pfMet_shiftedPt_JetCheckUESUp,row.type1_pfMet_shiftedPhi_JetCheckUESUp)
              taulorenz.SetPtEtaPhiM(row.tPt,row.tEta,row.tPhi,row.tMass) 
              #muonlorenz.SetPtEtaPhiM(row.mPt,row.mEta,row.mPhi,row.mMass) 
              metlorenz.SetPtEtaPhiM(self.tmpHula[0],0,self.tmpHula[3],0)
              self.collMass_type1_new=row.m_t_collinearmass_CheckUESUp#self.collMass_type1_v1(row,self.tmpHula[1],self.tmpHula[2])
              self.tMtToPfMet_type1_new=transverseMass_v2(taulorenz,metlorenz)
#              print "the mass from Ntuple %f and the mass from Cal %f" %(row.m_t_collinearmass_CheckUESUp,self.collMass_type1_new)
           if self.DoUES==4:
              self.tmpHula=Systematics.UESdown(row.type1_pfMet_shiftedPt_JetCheckUESDown,row.type1_pfMet_shiftedPhi_JetCheckUESDown)
              taulorenz.SetPtEtaPhiM(row.tPt,row.tEta,row.tPhi,row.tMass) 
              #muonlorenz.SetPtEtaPhiM(row.mPt,row.mEta,row.mPhi,row.mMass) 
              metlorenz.SetPtEtaPhiM(self.tmpHula[0],0,self.tmpHula[3],0)
              self.collMass_type1_new=row.m_t_collinearmass_CheckUESDown#self.collMass_type1_v1(row,self.tmpHula[1],self.tmpHula[2])
              self.tMtToPfMet_type1_new=transverseMass_v2(taulorenz,metlorenz)
    def VariableCalculateJES(self,row,JESShiftedMet,JESShiftedPhi):
           ...
           self.collMass_type1_new=self.collMass_type1_v1(row,self.tmpHulaJES[0]*math.cos(self.tmpHulaJES[1]),self.tmpHulaJES[0]*math.sin(self.tmpHulaJES[1]))
           self.tMtToPfMet_type1_new=transverseMass_v2(taulorenz,metlorenz)
...
#            print "Sysin value at line 13022 %f" %self.Sysin
     ...
     #                        self.fill_histos(row,tmp,False)
#"Iso#SS0Jet","IsoSS1Jet","IsoSS2Jet","ggIsoSS","boostIsoSS","vbfIsoSS","vbf_ggIsoSS","vbf_vbfIsoSS"
...
#    #                          self.fill_histos(row,'vbf_vbfNotIsoM2ndDown',True,"2ndDown",faketype="muonfake")
    ##        global Sysin
            self.Sysin=1
            if self.Sysin and (not self.is_data) :
                 sysneedI=['UESUpCheck','UESDownCheck']#,'MESUp','MESDown','TESUp','TESDown','UESUp','UESDown']
