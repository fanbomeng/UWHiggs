...
class AnalyzeLFVMuTau_progress_TES3_Fakshape_v3(MegaBase):
    tree = 'mt/final/Ntuple'
    #tree = 'New_Tree'

    def __init__(self, tree, outfile, **kwargs):
        super(AnalyzeLFVMuTau_progress_TES3_Fakshape_v3, self).__init__(tree, outfile, **kwargs)
        # Use the cython wrapper
        target = os.path.basename(os.environ['megatarget'])
        self.target1 = os.path.basename(os.environ['megatarget'])
        self.ls_recoilC=((('HTo' in target) or ('Jets' in target) or ('ttHJet') in target ) and MetCorrection) 
...
#           self.Z_reweight_H=self.Z_reweight.Get('zptmass_histo')
    def begin(self):
       ...
       #    name=[]
           sysneed=['MESUp','MESDown','TES0Up','TES0Down','TES1Up','TES1Down','TES10Up','TES10Down','UESUp','UESDown','MFTUp','MFTDown']
...
#           names=names+sysname
	...
	for x in range(0,namesize):
            ...
            self.book(names[x], "type1_pfMetPhi", "MetPhi", 200,-4,4)
...
#            met = math.sqrt(metpx*metpx+metpy*metpy)
  ...
  #            fTauIso=0.905855-0.0339542
    ...
    def MetCorrectionSet(self,row,MetValue,MetPhi,jetVeto30value):
           if self.ls_Wjets:
              ...
              tmpMet=self.Metcorected.CorrectByMeanResolution(MetValue*math.cos(MetPhi),MetValue*math.sin(MetPhi),row.genpX,row.genpY,row.vispX,row.vispY,int(round(jetVeto30value)))#,self.pfmetcorr_ex,self.pfmetcorr_ey)
              MetPhi=math.atan2(tmpMet[1],tmpMet[0])
              print "the Met befor %f and after %f" %(MetValue*math.cos(MetPhi),tmpMet[0])
           #MetPt4=[math.sqrt(self.tmpMet[0]*self.tmpMet[0]+self.tmpMet[1]*self.tmpMet[1]),self.tmpMet[0],self.tmpMet[1],0]
           return (math.sqrt(tmpMet[0]*tmpMet[0]+tmpMet[1]*tmpMet[1]),tmpMet[0],tmpMet[1],MetPhi)
...
#          print self.fakeRateMethod(row,fakeset)
          #print weight
     #     print "if the weight no lumi %f"  %(weight/35861.6953)
          weight=weight*self.fakeRateMethod(row,fakeset,faketype) #apply fakerate method for given isolation definition
       #   if self.DoFakeshapeDM:
       #      print "in fill histogram the Fakeshape value %f and the folder %s and weight value %f" %(self.DoFakeshapeDM,name,weight) 
       #   else:
       #      print "NNNNnfill histogram the Fakeshape value %f and the folder %s and weight value %f" %(self.DoFakeshapeDM,name,weight) 
...
#           print "fill histogram? %f" %row.mPt
           ...
           histos[name+'/type1_pfMetPhi'].Fill(self.MetPhi,weight)
...
#	histos[name+'/vbfJetVeto30'].Fill(row.vbfJetVeto30, weight)
 ...
 #       histos[name+'/vbfNJets30'].Fill(row.vbfNJets30, weight)
    ...
    def TauESC(self,row):
        if (not self.is_data) and (not self.ls_DY):
           if  row.tDecayMode==0:
    ...
    def VariableCalculateTaucorrection(self,row,tmp_tau_Pt_C,tmp_MET_tPtC,tmp_MET_PhiC):
...
#           print "muonlorenz %f %f %f" %(muonlorenz.Pt(),muonlorenz.Px(),muonlorenz.Py())
           metlorenz.SetPtEtaPhiM(tmp_MET_tPtC,0,tmp_MET_PhiC,0)
#           print "metlorenz %f %f %f" %(metlorenz.Pt(),metlorenz.Px(),metlorenz.Py())
    ...
    def VariableCalculate(self,row,tau_Pt_C,MET_tPtC,MET_PhiC):
           taulorenz=ROOT.TLorentzVector()
           muonlorenz=ROOT.TLorentzVector()             
           metlorenz=ROOT.TLorentzVector() 
           #print 'at line 871 the MES %f ' %self.DoMES
           if self.DoTES==1:
              tmpHulaTESUp=Systematics.TESup(tau_Pt_C,MET_tPtC,row.tPhi,MET_PhiC)
              taulorenz.SetPtEtaPhiM(tmpHulaTESUp[0],row.tEta,row.tPhi,row.tMass) 
              muonlorenz.SetPtEtaPhiM(row.mPt,row.mEta,row.mPhi,row.mMass) 
              metlorenz.SetPtEtaPhiM(tmpHulaTESUp[3],0,MET_PhiC,0)
              collMass_type1_new,m_t_Mass_new=self.collMass_type1_v2(row,muonlorenz,taulorenz,tmpHulaTESUp[4],tmpHulaTESUp[5])
              tMtToPfMet_type1_new=transverseMass_v2(taulorenz,metlorenz)
              return(collMass_type1_new,m_t_Mass_new,tMtToPfMet_type1_new,tmpHulaTESUp[0],tmpHulaTESUp[3],MET_PhiC)
           if self.DoTES==2:
              tmpHulaTESDown=Systematics.TESdown(tau_Pt_C,MET_tPtC,row.tPhi,MET_PhiC)
              taulorenz.SetPtEtaPhiM(tmpHulaTESDown[0],row.tEta,row.tPhi,row.tMass) 
              muonlorenz.SetPtEtaPhiM(row.mPt,row.mEta,row.mPhi,row.mMass) 
              metlorenz.SetPtEtaPhiM(tmpHulaTESDown[3],0,MET_PhiC,0)
              collMass_type1_new,m_t_Mass_new=self.collMass_type1_v2(row,muonlorenz,taulorenz,tmpHulaTESDown[4],tmpHulaTESDown[5])
              tMtToPfMet_type1_new=transverseMass_v2(taulorenz,metlorenz)
              return(collMass_type1_new,m_t_Mass_new,tMtToPfMet_type1_new,tmpHulaTESDown[0],tmpHulaTESDown[3],MET_PhiC)
           if self.DoTES==3: 
              taulorenz.SetPtEtaPhiM(tau_Pt_C,row.tEta,row.tPhi,row.tMass) 
              muonlorenz.SetPtEtaPhiM(row.mPt,row.mEta,row.mPhi,row.mMass) 
              metlorenz.SetPtEtaPhiM(MET_tPtC,0,MET_PhiC,0)
              collMass_type1_new,m_t_Mass_new=self.collMass_type1_v2(row,muonlorenz,taulorenz,MET_tPtC*math.cos(MET_PhiC),MET_tPtC*math.sin(MET_PhiC))
              tMtToPfMet_type1_new=transverseMass_v2(taulorenz,metlorenz)
              return(collMass_type1_new,m_t_Mass_new,tMtToPfMet_type1_new,tau_Pt_C,MET_tPtC,MET_PhiC)
           if self.DoMFT==1:
              tmpHulaMFTUp=Systematics.MFTup(tau_Pt_C,MET_tPtC,row.tPhi,MET_PhiC)
              taulorenz.SetPtEtaPhiM(tmpHulaMFTUp[0],row.tEta,row.tPhi,row.tMass) 
              muonlorenz.SetPtEtaPhiM(row.mPt,row.mEta,row.mPhi,row.mMass) 
              metlorenz.SetPtEtaPhiM(tmpHulaMFTUp[3],0,MET_PhiC,0)
              collMass_type1_new,m_t_Mass_new=self.collMass_type1_v2(row,muonlorenz,taulorenz,tmpHulaMFTUp[4],tmpHulaMFTUp[5])
              tMtToPfMet_type1_new=transverseMass_v2(taulorenz,metlorenz)
              return(collMass_type1_new,m_t_Mass_new,tMtToPfMet_type1_new,tmpHulaMFTUp[0],tmpHulaMFTUp[3],MET_PhiC)
           if self.DoMFT==2:
              tmpHulaMFTDown=Systematics.MFTdown(tau_Pt_C,MET_tPtC,row.tPhi,MET_PhiC)
              taulorenz.SetPtEtaPhiM(tmpHulaMFTDown[0],row.tEta,row.tPhi,row.tMass) 
              muonlorenz.SetPtEtaPhiM(row.mPt,row.mEta,row.mPhi,row.mMass) 
              metlorenz.SetPtEtaPhiM(tmpHulaMFTDown[3],0,MET_PhiC,0)
              collMass_type1_new,m_t_Mass_new=self.collMass_type1_v2(row,muonlorenz,taulorenz,tmpHulaMFTDown[4],tmpHulaMFTDown[5])
              tMtToPfMet_type1_new=transverseMass_v2(taulorenz,metlorenz)
              return(collMass_type1_new,m_t_Mass_new,tMtToPfMet_type1_new,tmpHulaMFTDown[0],tmpHulaMFTDown[3],MET_PhiC)
           if self.DoMFT==3: 
              taulorenz.SetPtEtaPhiM(tau_Pt_C,row.tEta,row.tPhi,row.tMass) 
              muonlorenz.SetPtEtaPhiM(row.mPt,row.mEta,row.mPhi,row.mMass) 
              metlorenz.SetPtEtaPhiM(MET_tPtC,0,MET_PhiC,0)
              ...
              return(collMass_type1_new,m_t_Mass_new,tMtToPfMet_type1_new,tau_Pt_C,MET_tPtC,MET_PhiC)
           if self.DoMES==1:
              tmpHulaMESUp=Systematics.MESup(row.mPt,row.mEta,MET_tPtC,row.mPhi,MET_PhiC)
              taulorenz.SetPtEtaPhiM(tau_Pt_C,row.tEta,row.tPhi,row.tMass) 
              muonlorenz.SetPtEtaPhiM(tmpHulaMESUp[0],row.mEta,row.mPhi,row.mMass) 
              metlorenz.SetPtEtaPhiM(tmpHulaMESUp[3],0,MET_PhiC,0)
              collMass_type1_new,m_t_Mass_new=self.collMass_type1_v2(row,muonlorenz,taulorenz,tmpHulaMESUp[4],tmpHulaMESUp[5])
              tMtToPfMet_type1_new=transverseMass_v2(taulorenz,metlorenz)
              return(collMass_type1_new,m_t_Mass_new,tMtToPfMet_type1_new,tmpHulaMESUp[0],tmpHulaMESUp[3],MET_PhiC)
           if self.DoMES==2:
              tmpHulaMESDown=Systematics.MESdown(row.mPt,row.mEta,MET_tPtC,row.mPhi,MET_PhiC)
              taulorenz.SetPtEtaPhiM(tau_Pt_C,row.tEta,row.tPhi,row.tMass) 
              muonlorenz.SetPtEtaPhiM(tmpHulaMESDown[0],row.mEta,row.mPhi,row.mMass) 
              metlorenz.SetPtEtaPhiM(tmpHulaMESDown[3],0,MET_PhiC,0)
              collMass_type1_new,m_t_Mass_new=self.collMass_type1_v2(row,muonlorenz,taulorenz,tmpHulaMESDown[4],tmpHulaMESDown[5])
              tMtToPfMet_type1_new=transverseMass_v2(taulorenz,metlorenz)
              return(collMass_type1_new,m_t_Mass_new,tMtToPfMet_type1_new,tmpHulaMESDown[0],tmpHulaMESDown[3],MET_PhiC)
           if self.DoUES==1:
              if not self.ls_DY:
                 if  row.tDecayMode==0:
                 ...
                 if self.ls_recoilC and MetCorrection:
                    RecoilMet=self.MetCorrectionSet(row,UES_shiftedMetup,row.type1_pfMet_shiftedPhi_UnclusteredEnUp,row.jetVeto30)
                 else:
                    RecoilMet=(UES_shiftedMetup,0,0,row.type1_pfMet_shiftedPhi_UnclusteredEnUp)
                 tmpHulaUESUp=Systematics.UESup(RecoilMet[0],RecoilMet[3])
                 taulorenz.SetPtEtaPhiM(tau_Pt_C,row.tEta,row.tPhi,row.tMass) 
                 muonlorenz.SetPtEtaPhiM(row.mPt,row.mEta,row.mPhi,row.mMass) 
                 ...
                 if row.isZmumu  and row.tZTTGenMatching<5 and row.tDecayMode==1:
                    UES_shiftedMetup=MET_tPtC-0.015*row.tPt
                 else: 
                    UES_shiftedMetup=MET_tPtC
                 if self.ls_recoilC and MetCorrection:
                    RecoilMet=self.MetCorrectionSet(row,UES_shiftedMetup,row.type1_pfMet_shiftedPhi_UnclusteredEnUp,row.jetVeto30)
                 else:
                    RecoilMet=(UES_shiftedMetup,0,0,row.type1_pfMet_shiftedPhi_UnclusteredEnUp)
                 ...
                 collMass_type1_new,m_t_Mass_new=self.collMass_type1_v2(row,muonlorenz,taulorenz,tmpHulaUESUp[1],tmpHulaUESUp[2])
                 tMtToPfMet_type1_new=transverseMass_v2(taulorenz,metlorenz)
              return(collMass_type1_new,m_t_Mass_new,tMtToPfMet_type1_new,tau_Pt_C,tmpHulaUESUp[0],tmpHulaUESUp[3])
           if self.DoUES==2:
              if not self.ls_DY:
                 if  row.tDecayMode==0:
                     UES_shiftedMetdown=MET_tPtC+0.018*row.tPt
                 if  row.tDecayMode==1:
                     UES_shiftedMetdown=MET_tPtC-0.01*row.tPt
                 if  row.tDecayMode==10:
                     UES_shiftedMetdown=MET_tPtC-0.004*row.tPt
                 if self.ls_recoilC and MetCorrection:
                    RecoilMet=self.MetCorrectionSet(row,UES_shiftedMetdown,row.type1_pfMet_shiftedPhi_UnclusteredEnDown,row.jetVeto30)
                 else:
                    RecoilMet=(UES_shiftedMetdown,0,0,row.type1_pfMet_shiftedPhi_UnclusteredEnDown)
                 #print "UES Met before %f and after correction %f" %(UES_shiftedMetdown,RecoilMet[0])
                 #print "UESX Met before %f and after correction %f" %(UES_shiftedMetdown*math.cos(row.type1_pfMet_shiftedPhi_UnclusteredEnDown),RecoilMet[1])
                 tmpHulaUESDown=Systematics.UESdown(RecoilMet[0],RecoilMet[3])
                 taulorenz.SetPtEtaPhiM(tau_Pt_C,row.tEta,row.tPhi,row.tMass) 
                 muonlorenz.SetPtEtaPhiM(row.mPt,row.mEta,row.mPhi,row.mMass) 
                 ...
                 tMtToPfMet_type1_new=transverseMass_v2(taulorenz,metlorenz)
                 return(collMass_type1_new,m_t_Mass_new,tMtToPfMet_type1_new,tau_Pt_C,tmpHulaUESDown[0],tmpHulaUESDown[3])
              else:
                 if row.isZmumu  and row.tZTTGenMatching<5 and row.tDecayMode==1:
                 ...
                 tMtToPfMet_type1_new=transverseMass_v2(taulorenz,metlorenz)
                 return(collMass_type1_new,m_t_Mass_new,tMtToPfMet_type1_new,tau_Pt_C,tmpHulaUESDown[0],tmpHulaUESDown[3])
    def VariableCalculateJES_UESsp(self,row,JESShiftedMet,JESShiftedPhi,JESjetveto30,tau_Pt_C):
           taulorenz=ROOT.TLorentzVector()
           muonlorenz=ROOT.TLorentzVector()             
           metlorenz=ROOT.TLorentzVector() 
           if not self.ls_DY:
              ...
              #print 'at line 871 the MES %f ' %self.DoMES
              if self.ls_recoilC and MetCorrection:
                 RecoilMet=self.MetCorrectionSet(row,JESShiftedMet_new,JESShiftedPhi,JESjetveto30)
              else:
                 RecoilMet=(JESShiftedMet_new,0,0,JESShiftedPhi)
              tmpHulaJES=(RecoilMet[0],RecoilMet[3])
              taulorenz.SetPtEtaPhiM(tau_Pt_C,row.tEta,row.tPhi,row.tMass) 
              muonlorenz.SetPtEtaPhiM(row.mPt,row.mEta,row.mPhi,row.mMass) 
              ...
              collMass_type1_new,m_t_Mass_new=self.collMass_type1_v2(row,muonlorenz,taulorenz,tmpHulaJES[0]*math.cos(tmpHulaJES[1]),tmpHulaJES[0]*math.sin(tmpHulaJES[1]))
              tMtToPfMet_type1_new=transverseMass_v2(taulorenz,metlorenz)
           return(collMass_type1_new,m_t_Mass_new,tMtToPfMet_type1_new,tau_Pt_C,tmpHulaJES[0],tmpHulaJES[1]) 
    def presel(self, row):
...
#            return False
  ...
  #      return  row.tByTightIsolationMVArun2v1DBoldDMwLT
    def SetsysZero(self,row):
        self.DoMES=0; self.DoTES=0; self.DoJES=0;self.DoUES=0;self.DoFakeshapeDM=0;self.DoMFT=0;self.DoUESsp=0
    def obj2_iso(self, row):
        return  row.tByTightIsolationMVArun2v1DBoldDMwLT
...
#            global Sysin
         #   print "********************************************************"
            ...
            if self.ls_recoilC and MetCorrection:
               RecoilMet=self.MetCorrectionSet(row,self.MET_tPtC,row.type1_pfMetPhi,row.jetVeto30)
#               print "Met before %f and after correction %f" %(self.MET_tPtC,RecoilMet[0])
               self.MET_tPtC=RecoilMet[0]
               self.MetPhi=RecoilMet[3]
            else:
               self.MetPhi=row.type1_pfMetPhi
            #print "one new event*****************************1"
            if self.kinematics(row):
                  self.collMass_type1_new,self.m_t_Mass_new,self.tMtToPfMet_type1_new=self.VariableCalculateTaucorrection(row,self.tau_Pt_C,self.MET_tPtC,self.MetPhi)
#                  print 'the original M_coll %f and the recoil ones %f' %(row.m_t_collinearmass,self.collMass_type1_new)
            ...
            if self.Sysin and not self.is_data:
                 for i in sysneedI:
                  ...
                  #       print " %s   %f" %(i,self.collMass_type1_new)
                     if self.obj2_iso(row) and self.oppositesign(row) and self.obj1_iso(row) and self.kinematics(row): 
                        self.collMass_type1_new,self.m_t_Mass_new,self.tMtToPfMet_type1_new,self.MorTPtshifted,self.MET_tPtCsys,self.MetPhisys=self.VariableCalculate(row,self.tau_Pt_C,self.MET_tPtC,self.MetPhi)
                 ...
                 if not self.ls_DY:
                    for L in sysneedTES:
                        ...
                        if self.obj2_iso(row) and self.oppositesign(row) and self.obj1_iso(row) and self.kinematics(row): 
                           self.collMass_type1_new,self.m_t_Mass_new,self.tMtToPfMet_type1_new,self.MorTPtshifted,self.MET_tPtCsys,self.MetPhisys=self.VariableCalculate(row,self.tau_Pt_C,self.MET_tPtC,self.MetPhi)
                           #print 'TES the original M_coll %f and the recoil ones %f' %(row.m_t_collinearmass,self.collMass_type1_new)
                           for j in basechannelsI: 
                              if  row.jetVeto30==j[0]:
                                  tmpname_2='self.'+j[1]+'(row)'
                                  if eval(tmpname_2):
...
#                        print "mass collin %s  %f" %(k,self.collMass_type1_new)
                     if self.obj2_iso(row) and self.oppositesign(row) and self.obj1_iso(row) and self.kinematics(row): 
                        self.collMass_type1_new,self.m_t_Mass_new,self.tMtToPfMet_type1_new,self.MorTPtshifted,self.MET_tPtCsys,self.MetPhisys=self.VariableCalculateJES_UESsp(row,eval(tmpname_5),eval(tmpname_6),eval(tmpname_3),self.tau_Pt_C)
                        for j in basechannelsI: 
...
#                        print "mass collin %s  %f" %(k,self.collMass_type1_new)
                     if self.obj2_iso(row) and self.oppositesign(row) and self.obj1_iso(row) and self.kinematics(row): 
                        self.collMass_type1_new,self.m_t_Mass_new,self.tMtToPfMet_type1_new,self.MorTPtshifted,self.MET_tPtCsys,self.MetPhisys=self.VariableCalculateJES_UESsp(row,eval(tmpname_5),eval(tmpname_6),row.jetVeto30,self.tau_Pt_C)
                        for j in basechannelsI: 
                           if  row.jetVeto30==j[0]:
                               tmpname_2='self.'+j[1]+'(row)'
                               if  eval(tmpname_2):
