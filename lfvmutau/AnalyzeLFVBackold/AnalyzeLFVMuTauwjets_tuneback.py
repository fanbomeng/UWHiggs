...
class AnalyzeLFVMuTauwjets_tune(MegaBase):
    tree = 'mt/final/Ntuple'
    #tree = 'New_Tree'

    def __init__(self, tree, outfile, **kwargs):
        super(AnalyzeLFVMuTauwjets_tune, self).__init__(tree, outfile, **kwargs)
...
#cancled channals "preselection","preselectionSS", "notIso","notIsoNotWeightedSS","notIsoSS","notIsoNotWeighted"
        ...
        if (not fakeset) and (not wjets_fakes):
           ...
           names=["gg","boost","vbf","ggNotIso","boostNotIso","vbfNotIso","preselection0Jet", "preselection1Jet", "preselection2Jet","notIso0Jet", "notIso1Jet","notIso2Jet","vbf_gg","vbf_vbf","vbf_ggNotIso","vbf_vbfNotIso"]
        if wjets_fakes:
            ...
            if fakeset or wjets_fakes:
               if self.obj2_iso(row) and not self.oppositesign(row):
                  ...
                  if row.jetVeto30==2:
                    ...
                    if RUN_OPTIMIZATION:
                       for  i in optimizer.compute_regions_2jettight(row.tPt, row.mPt,row.tMtToPfMet_type1,row.vbfMass,row.vbfDeta):
                          tmp=os.path.join("vbf_vbfIsoSS",i)
                          self.fill_histos(row,tmp,False)
                    if self.vbf_gg(row):
                       self.fill_histos(row,'vbf_ggIsoSS',False)
                    if RUN_OPTIMIZATION:
                       for  i in optimizer.compute_regions_2jetloose(row.tPt, row.mPt,row.tMtToPfMet_type1,row.vbfMass,row.vbfDeta):
