...
class AnalyzeLFVMuTau_Fakestudy(MegaBase):
    tree = 'mt/final/Ntuple'
    #tree = 'New_Tree'

    def __init__(self, tree, outfile, **kwargs):
        super(AnalyzeLFVMuTau_Fakestudy, self).__init__(tree, outfile, **kwargs)
...
#cancled channals "preselection","preselectionSS", "notIso","notIsoNotWeightedSS","notIsoSS","notIsoNotWeighted"
        if fakeset  :
           names=["preselection","notIso","preselectionSS","notIsoSS","preselectionEnWjets","notIsoEnWjets","preselectionSSEnWjets","notIsoEnWjetsSS","preselectionMP","preselectionMPSS","preselectionMM","preselectionMMSS","preselectionTP","preselectionTPSS","preselectionTM","preselectionTMSS","gg","boost","vbf","ggNotIso","boostNotIso","ggNotIso1stUp","ggNotIso1stDown","boostNotIso1stUp","boostNotIso1stDown","ggNotIso2ndUp","ggNotIso2ndDown","boostNotIso2ndUp","boostNotIso2ndDown","vbfNotIso","preselection0Jet", "preselection1Jet", "preselection2Jet","notIso0Jet", "notIso1Jet","notIso2Jet","vbf_gg","vbf_vbf","vbf_ggNotIso","vbf_vbfNotIso","vbf_ggNotIso1stUp","vbf_ggNotIso1stDown","vbf_vbfNotIso1stUp","vbf_vbfNotIso1stDown","vbf_ggNotIso2ndUp","vbf_ggNotIso2ndDown","vbf_vbfNotIso2ndUp","vbf_vbfNotIso2ndDown","IsoSS0Jet","IsoSS1Jet","IsoSS2Jet","ggIsoSS","boostIsoSS","vbfIsoSS","vbf_ggIsoSS","vbf_vbfIsoSS","SLSS","SLnotIsoSS","SLEnWjets","SLEnWjetsnotIso","SLEnWjetsSS","SLEnWjetsnotIsoSS","Zmumu_preselection0Jet","Zmumu_preselection0JetnotIso","Zmumu_preselection1Jet","Zmumu_preselection1JetnotIso","Zmumu_preselection2Jet","Zmumu_preselection2JetnotIso","Zmumu_gg","Zmumu_ggnotIso","Zmumu_boost","Zmumu_boostnotIso","Zmumu_vbf_gg","Zmumu_vbf_ggnotIso","Zmumu_vbf_vbf","Zmumu_vbf_vbfnotIso","Zll_preselection0Jet","Zll_preselection0JetnotIso","Zll_preselection1Jet","Zll_preselection1JetnotIso","Zll_preselection2Jet","Zll_preselection2JetnotIso","Zll_gg","Zll_ggnotIso","Zll_boost","Zll_boostnotIso","Zll_vbf_gg","Zll_vbf_ggnotIso","Zll_vbf_vbf","Zll_vbf_vbfnotIso"]
               ...
               if self.obj2_iso(row) and not self.oppositesign(row):
                  if fakeset:
                     if self.WjetsEnrich(row):
                        self.fill_histos(row,'preselectionSSEnWjets',False)
                     self.fill_histos(row,'preselectionSS',False)
                     if row.mCharge==1:
                        self.fill_histos(row,'preselectionMPSS',False)
                     if row.mCharge==-1:
                        self.fill_histos(row,'preselectionMMSS',False)
                     if row.tCharge==1:
                        self.fill_histos(row,'preselectionTPSS',False)
                     if row.tCharge==-1:
                        self.fill_histos(row,'preselectionTMSS',False)
                  if row.jetVeto30==0:
                    self.fill_histos(row,'IsoSS0Jet',False)
                    if self.gg(row):
                       self.fill_histos(row,'ggIsoSS',False)
                       self.fill_histos(row,'SLSS',False)
                       if self.WjetsEnrich(row):
                          self.fill_histos(row,'SLEnWjetsSS',False)
                    if RUN_OPTIMIZATION:
...
#              print row.m_t_collinearmass
              if fakeset:
                 ...
                 if self.WjetsEnrich(row):
                    self.fill_histos(row,'preselectionEnWjets',False)

                 if row.mCharge==1:
                    self.fill_histos(row,'preselectionMP',False)
                 if row.mCharge==-1:
                    self.fill_histos(row,'preselectionMM',False)
                 if row.tCharge==1:
                    self.fill_histos(row,'preselectionTP',False)
                 if row.tCharge==-1:
                    self.fill_histos(row,'preselectionTM',False)
              ...
              if row.jetVeto30==0:
                ...
                if row.isZmumu==1:
                   self.fill_histos(row,'Zmumu_preselection0Jet',False)
                else: 
                   self.fill_histos(row,'Zll_preselection0Jet',False)
              ...
              if row.jetVeto30==1:
                ...
                if row.isZmumu==1:
                   self.fill_histos(row,'Zmumu_preselection1Jet',False)
                else:
                   self.fill_histos(row,'Zll_preselection1Jet',False)
                if wjets_fakes and row.isWmunu==1:
              ...
              if row.jetVeto30==2:
                ...
                if row.isZmumu==1:
                   self.fill_histos(row,'Zmumu_preselection2Jet',False)
                else:
                   self.fill_histos(row,'Zll_preselection2Jet',False)
                if wjets_fakes and row.isWmunu==1:
              ...
              if  row.jetVeto30==0:
		        ...
		        self.fill_histos(row,tmp,False)
                  if self.gg(row):
                        self.fill_histos(row,'gg',False)
                        if row.isZmumu==1:
                           self.fill_histos(row,'Zmumu_gg',False)
                        else:
                           self.fill_histos(row,'Zll_gg',False)
                        if self.WjetsEnrich(row):
                           self.fill_histos(row,'SLEnWjets',False)
                        if wjets_fakes and row.isWmunu==1:
              ...
              if row.jetVeto30==1:
		        ...
		        self.fill_histos(row,tmp,False)	
                  if self.boost(row):
                        self.fill_histos(row,'boost',False)
                        if row.isZmumu==1:
                           self.fill_histos(row,'Zmumu_boost',False)
                        else:
                           self.fill_histos(row,'Zll_boost',False)
                        if self.WjetsEnrich(row):
                           self.fill_histos(row,'SLEnWjets',False)
                        if wjets_fakes and row.isWmunu==1:
...
#		        self.fill_histos(row,tmp,False)	
                  ...
                  if self.vbf_gg(row):
                        ...
                        if row.isZmumu==1:
                           self.fill_histos(row,'Zmumu_vbf_gg',False)
                        else:
                           self.fill_histos(row,'Zll_vbf_gg',False)
                        if self.WjetsEnrich(row):
                           self.fill_histos(row,'SLEnWjets',False)
                        if wjets_fakes and row.isWmunu==1:
                  ...
                  if self.vbf_vbf(row):
                        ...
                        if row.isZmumu==1:
                           self.fill_histos(row,'Zmumu_vbf_vbf',False)
                        else:
                           self.fill_histos(row,'Zll_vbf_vbf',False)
                        if self.WjetsEnrich(row):
                           self.fill_histos(row,'SLEnWjets',False)
                        if wjets_fakes and row.isWmunu==1:
...
#              self.fill_histos(row,'notIsoNotWeighted',False)
              ...
              if row.jetVeto30==0:
                ...
                if row.isZmumu==1:
                   self.fill_histos(row,'Zmumu_preselection0JetnotIso',True)
                else:
                   self.fill_histos(row,'Zll_preselection0JetnotIso',True)
              if row.jetVeto30==1:
                self.fill_histos(row,'notIso1Jet',True)
                if row.isZmumu==1:
                   self.fill_histos(row,'Zmumu_preselection1JetnotIso',True)
                else:
                   self.fill_histos(row,'Zll_preselection1JetnotIso',True)
              if row.jetVeto30==2:
                self.fill_histos(row,'notIso2Jet',True)
                if row.isZmumu==1:
                   self.fill_histos(row,'Zmumu_preselection2JetnotIso',True)
                else:
                   self.fill_histos(row,'Zll_preselection2JetnotIso',True)
              if  row.jetVeto30==0:
                 # #if RUN_OPTIMIZATION:
                 ...
                 # #      self.fill_histos(row,tmp,True)
                  if self.gg(row):
                        ...
                        if row.isZmumu==1:
                           self.fill_histos(row,'Zmumu_ggnotIso',True)
                        else:
                           self.fill_histos(row,'Zll_ggnotIso',True)
                        if self.WjetsEnrich(row):
                           self.fill_histos(row,'SLEnWjetsnotIso',True)
                        if fakeset:
              ...
              if row.jetVeto30==1:
               ...
               #  #       self.fill_histos(row,tmp,True)
                  if self.boost(row):
                        ...
                        if row.isZmumu==1:
                           self.fill_histos(row,'Zmumu_boostnotIso',True)
                        else:
                           self.fill_histos(row,'Zll_boostnotIso',True)
                        if self.WjetsEnrich(row):
                           self.fill_histos(row,'SLEnWjetsnotIso',True)
                        if fakeset:
         ...
         #         self.fill_histos(row,'boostNotIso',True)
            ...
            #  #          self.fill_histos(row,tmp,True)
                  ...
                  if self.vbf_gg(row):
                        ...
                        if row.isZmumu==1:
                           self.fill_histos(row,'Zmumu_vbf_ggnotIso',True)
                        else:
                           self.fill_histos(row,'Zll_vbf_ggnotIso',True)
                        if self.WjetsEnrich(row):
                           self.fill_histos(row,'SLEnWjetsnotIso',True)
                        if fakeset:
                  ...
                  if self.vbf_vbf(row):
                        ...
                        if row.isZmumu==1:
                           self.fill_histos(row,'Zmumu_vbf_vbfnotIso',True)
                        else:
                           self.fill_histos(row,'Zll_vbf_vbfnotIso',True)
                        if self.WjetsEnrich(row):
                           self.fill_histos(row,'SLEnWjetsnotIso',True)
                        if fakeset:
...
#                  self.fill_histos(row,'vbfNotIso',True)
            if self.obj2_iso_NT_VLoose(row) and not self.oppositesign(row):
                if row.jetVeto30==0:
                    if self.gg(row):
                       self.fill_histos(row,'SLnotIsoSS',True)
                       if self.WjetsEnrich(row):
                          self.fill_histos(row,'SLEnWjetsnotIsoSS',True)
                if row.jetVeto30==1:
                    if self.boost(row):
                       self.fill_histos(row,'SLnotIsoSS',True)
                       if self.WjetsEnrich(row):
