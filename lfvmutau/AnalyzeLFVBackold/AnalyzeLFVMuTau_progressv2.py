...
def getFakeRateFactormuon(row, fakeset):
     if fakeset=="def":
...
#            self.book(names[x], "vbfDphihjnomet","",100,0,4)
    ...
    def fakeRateMethod(self,row,fakeset,faketype):
        ...
        if faketype=="mtfake":
           return getFakeRateFactormuon(row,fakeset)*getFakeRateFactorAaron(row,fakeset)
       # return getFakeRateFactorAaron(row,fakeset)
...
#          histos['jetPt'].Fill(1,1)
    def fill_histos(self, row,name='gg', fakeRate=False,fakeset="def",faketype="taufake"):
        histos = self.histograms
	   ...
	   weight = row.GenWeight * self.correction(row)*bTagSF.bTagEventWeight(row.bjetCISVVeto30Medium,row.jb1pt,row.jb1hadronflavor,row.jb2pt,row.jb2hadronflavor,1,btagSys,0)
...
#                continue
  ...
  #              continue 
            print "pass normal check-6"
            if not self.obj1_isoloose(row):
                continue
            print "pass normal check-5"
            if not self.obj1_id(row):
                continue
            print "pass normal check-4"
         #   if not self.obj1_idICHEP(row):
         #       continue
            if not self.vetos (row):
                continue

            print "pass normal check-3"
            if not self.obj2_Vlooseiso(row):
                continue
            print "pass normal check-2"
            if (self.is_data):
               if  row.bjetCISVVeto30Medium:
                   continue
            print "pass normal check-1"
            if not self.obj2_id (row):
                continue
    
            print "pass normal check1"
...
#              print row.m_t_collinearmass
             print "pass normal check2"
             if self.obj1_iso(row):
              print "pass normal check3"
              if fakeset:
