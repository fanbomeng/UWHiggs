...
class AnalyzeLFVMuTauPostBDT_progress_v7(MegaBase):
    tree = 'mt/final/Ntuple'
    #tree = 'New_Tree'

    def __init__(self, tree, outfile, **kwargs):
        super(AnalyzeLFVMuTauPostBDT_progress_v7, self).__init__(tree, outfile, **kwargs)
...
#           print "metlorenz %f %f %f" %(metlorenz.Pt(),metlorenz.Px(),metlorenz.Py())
    ...
    def VariableCalculate(self,row,tau_Pt_C,MET_tPtC):
           ...
           if self.DoUES==1:
              if not self.ls_DY:
                 ...
                 tmpHulaUESUp=Systematics.UESup(UES_shiftedMetup,row.type1_pfMetPhi)
                 taulorenz.SetPtEtaPhiM(tau_Pt_C,row.tEta,row.tPhi,row.tMass) 
                 muonlorenz.SetPtEtaPhiM(row.mPt,row.mEta,row.mPhi,row.mMass) 
                 ...
                 var_d_0  ={'mPt_':row.mPt,'tPt_':tau_Pt_C,'tMtToPfMet_type1_':tMtToPfMet_type1_new,'m_t_DPhi_':abs(row.m_t_DPhi),'tDPhiToPfMet_type1_':abs(row.tDPhiToPfMet_type1),'type1_pfMetEt_':tmpHulaUESUp[0],'deltaeta_m_t_':abs(row.mEta-row.tEta),'m_t_collinearmass_':collMass_type1_new}
                 ...
                 tmpHulaUESUp=Systematics.UESup(row.type1_pfMet_shiftedPt_UnclusteredEnUp,row.type1_pfMetPhi)
                 ...
                 tMtToPfMet_type1_new=transverseMass_v2(taulorenz,metlorenz)
                 mMtToPfMet_type1_new=transverseMass_v2(muonlorenz,metlorenz)
                 var_d_0  ={'mPt_':row.mPt,'tPt_':row.tPt,'tMtToPfMet_type1_':tMtToPfMet_type1_new,'m_t_DPhi_':abs(row.m_t_DPhi),'tDPhiToPfMet_type1_':abs(row.tDPhiToPfMet_type1),'type1_pfMetEt_':tmpHulaUESUp[0],'deltaeta_m_t_':abs(row.mEta-row.tEta),'m_t_collinearmass_':collMass_type1_new}
           ...
           if self.DoUES==2:
              if not self.ls_DY:
                 ...
                 tmpHulaUESDown=Systematics.UESdown(UES_shiftedMetdown,row.type1_pfMetPhi)
                 taulorenz.SetPtEtaPhiM(tau_Pt_C,row.tEta,row.tPhi,row.tMass) 
                 muonlorenz.SetPtEtaPhiM(row.mPt,row.mEta,row.mPhi,row.mMass) 
                 ...
                 var_d_0  ={'mPt_':row.mPt,'tPt_':tau_Pt_C,'tMtToPfMet_type1_':tMtToPfMet_type1_new,'m_t_DPhi_':abs(row.m_t_DPhi),'tDPhiToPfMet_type1_':abs(row.tDPhiToPfMet_type1),'type1_pfMetEt_':tmpHulaUESDown[0],'deltaeta_m_t_':abs(row.mEta-row.tEta),'m_t_collinearmass_':collMass_type1_new}
                 MVA0fill_new=self.functor(**var_d_0)
                 return(MVA0fill_new,collMass_type1_new,tMtToPfMet_type1_new,mMtToPfMet_type1_new,tmpHulaUESDown[0])
              else:
                 tmpHulaUESDown=Systematics.UESdown(row.type1_pfMet_shiftedPt_UnclusteredEnDown,row.type1_pfMetPhi)
                 taulorenz.SetPtEtaPhiM(row.tPt,row.tEta,row.tPhi,row.tMass) 
                 muonlorenz.SetPtEtaPhiM(row.mPt,row.mEta,row.mPhi,row.mMass) 
                 metlorenz.SetPtEtaPhiM(tmpHulaUESDown[0],0,tmpHulaUESDown[3],0)
                 collMass_type1_new=self.collMass_type1_v2(row,muonlorenz,taulorenz,tmpHulaUESDown[1],tmpHulaUESDown[2])
                 tMtToPfMet_type1_new=transverseMass_v2(taulorenz,metlorenz)
                 mMtToPfMet_type1_new=transverseMass_v2(muonlorenz,metlorenz)
                 var_d_0  ={'mPt_':row.mPt,'tPt_':row.tPt,'tMtToPfMet_type1_':tMtToPfMet_type1_new,'m_t_DPhi_':abs(row.m_t_DPhi),'tDPhiToPfMet_type1_':abs(row.tDPhiToPfMet_type1),'type1_pfMetEt_':tmpHulaUESDown[0],'deltaeta_m_t_':abs(row.mEta-row.tEta),'m_t_collinearmass_':collMass_type1_new}
                 MVA0fill_new=self.functor(**var_d_0)
                 return(MVA0fill_new,collMass_type1_new,tMtToPfMet_type1_new,mMtToPfMet_type1_new,tmpHulaUESDown[0])
    def VariableCalculateJES(self,row,JESShiftedMet,JESShiftedPhi,tau_Pt_C): 
           ...
           if not self.ls_DY:
              ...
              #print 'at line 871 the MES %f ' %self.DoMES
              tmpHulaJES=(JESShiftedMet_new,row.type1_pfMetPhi)
              taulorenz.SetPtEtaPhiM(tau_Pt_C,row.tEta,row.tPhi,row.tMass) 
              muonlorenz.SetPtEtaPhiM(row.mPt,row.mEta,row.mPhi,row.mMass) 
              ...
              tMtToPfMet_type1_new=transverseMass_v2(taulorenz,metlorenz)
              mMtToPfMet_type1_new=transverseMass_v2(muonlorenz,metlorenz)
              var_d_0  ={'mPt_':row.mPt,'tPt_':tau_Pt_C,'tMtToPfMet_type1_':tMtToPfMet_type1_new,'m_t_DPhi_':abs(row.m_t_DPhi),'tDPhiToPfMet_type1_':abs(row.tDPhiToPfMet_type1),'type1_pfMetEt_':metlorenz.Pt(),'deltaeta_m_t_':abs(row.mEta-row.tEta),'m_t_collinearmass_':collMass_type1_new}
           else:
              tmpHulaJES=(JESShiftedMet,row.type1_pfMetPhi)
              taulorenz.SetPtEtaPhiM(tau_Pt_C,row.tEta,row.tPhi,row.tMass) 
              muonlorenz.SetPtEtaPhiM(row.mPt,row.mEta,row.mPhi,row.mMass) 
              metlorenz.SetPtEtaPhiM(tmpHulaJES[0],0,tmpHulaJES[1],0)
              collMass_type1_new=self.collMass_type1_v2(row,muonlorenz,taulorenz,tmpHulaJES[0]*math.cos(tmpHulaJES[1]),tmpHulaJES[0]*math.sin(tmpHulaJES[1]))
              tMtToPfMet_type1_new=transverseMass_v2(taulorenz,metlorenz)
              mMtToPfMet_type1_new=transverseMass_v2(muonlorenz,metlorenz)
              var_d_0  ={'mPt_':row.mPt,'tPt_':row.tPt,'tMtToPfMet_type1_':tMtToPfMet_type1_new,'m_t_DPhi_':abs(row.m_t_DPhi),'tDPhiToPfMet_type1_':abs(row.tDPhiToPfMet_type1),'type1_pfMetEt_':metlorenz.Pt(),'deltaeta_m_t_':abs(row.mEta-row.tEta),'m_t_collinearmass_':collMass_type1_new}
