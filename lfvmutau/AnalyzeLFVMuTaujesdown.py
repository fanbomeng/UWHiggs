...
#           tmpMet=self.Metcorected.CorrectByMeanResolution(row.type1_pfMet_shiftedPt_JetEnDown*math.cos(row.type1_pfMet_shiftedPhi_JetEnDown),row.type1_pfMetEt*math.sin(row.type1_pfMetPhi),row.genpX,row.genpY,row.vispX,row.vispY,int(round(row.jetVeto30_JetEnDown)))#,self.pfmetcorr_ex,self.pfmetcorr_ey)
...
#	histos[name+'/vbfJetVeto30'].Fill(row.vbfJetVeto30_JetEnDown, weight)
        ...
        histos[name+'/vbfNJets30'].Fill(row.vbfNJets30_JetEnDown, weight)
        #histos[name+'/vbfNJets30PULoose'].Fill(row.vbfNJets30_JetEnDownPULoose, weight)
        #histos[name+'/vbfNJets30PUTight'].Fill(row.vbfNJets30_JetEnDownPUTight, weight)
...
#	if(row.vbfNJets30_JetEnDown<2):
#	    return False
	#if (abs(row.vbfDeta_JetEnDown)>3.5 and row.vbfMass_JetEnDown > 550):   #was 2.5    #newcut 2.0
	if (row.vbfMass_JetEnDown > 550):   #was 2.5    #newcut 2.0
	    return False
#        if row.vbfMass_JetEnDown > 550:    #was 20   newcut 240
#	    return False
#        if row.vbfMass_JetEnDown < 100:    #was 20   newcut 240
#	    return False
  #      if row.vbfJetVeto30_JetEnDown > 0:
  #          return False
#        if row.bjetCISVVeto30Medium:
