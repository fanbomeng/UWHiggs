...
def make_muon_pog_IsoMu24oIsoTkMu24_2016ReRecoeffi():
...
def make_muon_pog_Mu50orTkMu50_2016ReRecoeffi():

    ''' trigger efficiencies in DATA; weigthed by lumi for two sets available , viz Run273158_to_274093(621.9 /pb) and Run274094_to_276097 (7036.4 /pb); more info in MUON POG twiki '''

    return MuonPOGCorrectionTrig2D_ReReco(
            _DATA_FILES['2016ReReco']['Trigger'],
            "Mu50_OR_TkMu50_PtEtaBins/efficienciesDATA/pt_abseta_DATA",19.72,16.15
        )

def make_muon_pog_Mu50orTkMu50_2016ReRecoSF():

    ''' trigger efficiencies in DATA; weigthed by lumi for two sets available , viz Run273158_to_274093(621.9 /pb) and Run274094_to_276097 (7036.4 /pb); more info in MUON POG twiki '''

    return MuonPOGCorrectionTrig2D_ReReco(
            _DATA_FILES['2016ReReco']['Trigger'],
            "Mu50_OR_TkMu50_PtEtaBins/pt_abseta_ratio",19.72,16.15
        )
...
def make_muon_pog_IsoMu50oIsoTkMu50_2016ReReco():

    ''' trigger efficiencies in DATA; weigthed by lumi for two sets available , viz Run273158_to_274093(621.9 /pb) and Run274094_to_276097 (7036.4 /pb); more info in MUON POG twiki '''

    return MuonPOGCorrectionTrig2D_ReReco(
            _DATA_FILES['2016ReReco']['Trigger'],
            "IsoMu50_OR_IsoTkMu50_PtEtaBins/pt_abseta_ratio",19.72,16.15
        )
...
def make_muon_pog_PFMedium1D_2016ReReco():
    ''' Muon ID SFs ReReco lumi '''
    return MuonPOGCorrectionID1D_ReReco(
        _DATA_FILES['2016ReReco']['PFID'],
        "MC_NUM_MediumID_DEN_genTracks_PAR_pt/pt_ratio",19.72,16.15 
...
def make_muon_pog_LooseIso1D_2016ReReco(MuonID):
   '''MuonID has to be a string - {Tight}{Loose}{Medium} '''
   return MuonPOGCorrectionIso1D_ReReco(
       MuonID,
       _DATA_FILES['2016ReReco']['Iso'],
      [ "LooseISO_LooseID_pt/pt_ratio",
       "LooseISO_MediumID_pt/pt_ratio",
       "LooseISO_TightID_pt/pt_ratio",],19.72,16.15 
                
    )


def make_muon_pog_TightIso1D_2016ReReco(MuonID):
    ...
    return MuonPOGCorrectionIso1D_ReReco(
        MuonID,
        _DATA_FILES['2016ReReco']['Iso'],
       [ "TightISO_LooseID_pt/pt_ratio",
         "TightISO_MediumID_pt/pt_ratio",
         "TightISO_TightID_pt/pt_ratio"],19.72,16.15 
...
#        self.correct_by_pt_abseta_weighted =(1.899*self.correct_by_pt_abseta_4p3+0.401*self.correct_by_pt_abseta_4p2)/2.3
        self.correct_by_pt_abseta_weighted =(self.lumi1*self.correct_by_pt_abseta1+self.lumi2*self.correct_by_pt_abseta2)/(self.lumi1+self.lumi2)
        return self.correct_by_pt_abseta_weighted
class MuonPOGCorrectionID1D_ReReco(object):
    '''
    Muon POG corrections are generally by eta dependent for pt > 20,
    and pt dependent for pt < 20, split by barrel, overlap and endcap.
    ...
    def __init__(self, files, pt,lumi1,lumi2):
        self.file1 = ROOT.TFile.Open(files[0])
        self.file2 = ROOT.TFile.Open(files[1])
        #self.pt_thr  = pt_thr
        self.lumi1=lumi1
    ...
    def __call__(self, pt):
        if pt >= 200: pt = 199.
        if pt < 20. : pt = 20.
        self.correct_by_pt1 =self.key1.GetBinContent(self.key1.FindFixBin(pt))
        self.correct_by_pt2 =self.key2.GetBinContent(self.key2.FindFixBin(pt))
#        self.correct_by_pt_abseta_weighted =(1.899*self.correct_by_pt_abseta_4p3+0.401*self.correct_by_pt_abseta_4p2)/2.3
        self.correct_by_pt_weighted =(self.lumi1*self.correct_by_pt1+self.lumi2*self.correct_by_pt2)/(self.lumi1+self.lumi2)
        return self.correct_by_pt_weighted
...
class MuonPOGCorrectionIso1D_ReReco(object):
    '''
    Muon POG corrections are generally by eta dependent for pt > 20,
    and pt dependent for pt < 20, split by barrel, overlap and endcap.
    ...
    def __init__(self,MuonID, files, pt,lumi1,lumi2):
        self.file1 = ROOT.TFile.Open(files[0])
        self.file2 = ROOT.TFile.Open(files[1])
        #self.pt_thr  = pt_thr
        self.lumi1=lumi1
