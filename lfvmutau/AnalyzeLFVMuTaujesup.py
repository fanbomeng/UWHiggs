...
class AnalyzeLFVMuTaujesup(MegaBase):
    ...
    def __init__(self, tree, outfile, **kwargs):
     ...
     #   print "*************"
     #   print self.is_data
        self.is_ZeroJet=('WJetsToLNu' in target)
        self.is_OneJet=('W1JetsToLNu' in target)
        self.is_TwoJet=('W2JetsToLNu' in target)
        self.is_ThreeJet=('W3JetsToLNu' in target)
        self.is_FourJet=('W4JetsToLNu' in target)
...
##        histos[name+'/eVetoMVAIso'].Fill(row.eVetoMVAIso, weight)
    ...
    def gg(self,row):
       if row.mPt < 25:   #was45 
           return False
       if deltaPhi(row.mPhi, row.tPhi) <2.7:  # was 2.7
           return False
       if row.tPt < 30:  #was 35
           return False
       if row.tMtToPfMet_JetEnUp > 65:  #was 50
           return False
    ...
    def boost(self,row):
          ...
          if row.tPt < 30:  #was 40
                return False
          if row.tMtToPfMet_JetEnUp > 75: #was 35
                return False
          return True
    ...
    def vbf(self,row):
       ...
       # if row.mPt < 30:
       # 	return False
        if row.tMtToPfMet_JetEnUp > 55: #was 35
                return False
        if row.jetVeto30_JetEnUp<2:  
            return False
	if(row.vbfNJets_JetEnUp<2):
	    return False
	if(abs(row.vbfDeta_JetEnUp)<2.0):   #was 2.5
	    return False
        if row.vbfMass_JetEnUp < 325:    #was 200
	    return False
