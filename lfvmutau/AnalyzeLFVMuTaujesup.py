...
#	metE = row.type1_pfMet_shiftedPt_JetEnUp
#	metPhi = row.type1_pfMet_shiftedPhi_JetEnUp
...
class AnalyzeLFVMuTaujesup(MegaBase):
    tree = 'mt/final/Ntuple'
    #tree = 'New_Tree'

    def __init__(self, tree, outfile, **kwargs):
        super(AnalyzeLFVMuTaujesup, self).__init__(tree, outfile, **kwargs)
...
#           tmpMet=self.Metcorected.CorrectByMeanResolution(row.type1_pfMet_shiftedPt_JetEnUp*math.cos(row.type1_pfMet_shiftedPhi_JetEnUp),row.type1_pfMetEt*math.sin(row.type1_pfMetPhi),row.genpX,row.genpY,row.vispX,row.vispY,int(round(row.jetVeto30_JetEnUp)))#,self.pfmetcorr_ex,self.pfmetcorr_ey)
...
#	histos[name+'/mDPhiToPfMet_tDPhiToPfMet'].Fill(abs(row.mDPhiToPfMet_JetEnUp),abs(row.tDPhiToPfMet_JetEnUp),weight)
#	histos[name+'/mDPhiToPfMet_ggdeltaphi'].Fill(abs(row.mDPhiToPfMet_JetEnUp),deltaPhi(row.mPhi, row.tPhi),weight)
#	histos[name+'/tDPhiToPfMet_ggdeltaphi'].Fill(abs(row.tDPhiToPfMet_JetEnUp),deltaPhi(row.mPhi, row.tPhi),weight)
#	histos[name+'/tDPhiToPfMet_tMtToPfMet_type1'].Fill(abs(row.tDPhiToPfMet_JetEnUp),row.tMtToPfMet_JetEnUp,weight)
#	histos[name+'/vbfmass_vbfdeta'].Fill(row.vbfMass_JetEnUp,abs(row.vbfDeta_JetEnUp),weight)
...
##        histos[name+'/jetVeto30'].Fill(row.jetVeto30_JetEnUp, weight)
##        histos[name+'/jetVeto30Eta3'].Fill(row.jetVeto30Eta3_JetEnUp,weight)
        #histos[name+'/jetVeto30PUCleanedLoose'].Fill(row.jetVeto30_JetEnUpPUCleanedLoose, weight)
        #histos[name+'/jetVeto30PUCleanedTight'].Fill(row.jetVeto30_JetEnUpPUCleanedTight, weight)
...
#	histos[name+'/mRelPFIsoDBDefault'].Fill(row.mRelPFIsoDBDefault, weight)
	...
	histos[name+'/tDecayMode'].Fill(row.tDecayMode, weight)
#	histos[name+'/vbfJetVeto30'].Fill(row.vbfJetVeto30_JetEnUp, weight)
        ...
        histos[name+'/vbfNJets30'].Fill(row.vbfNJets30_JetEnUp, weight)
        #histos[name+'/vbfNJets30PULoose'].Fill(row.vbfNJets30_JetEnUpPULoose, weight)
        #histos[name+'/vbfNJets30PUTight'].Fill(row.vbfNJets30_JetEnUpPUTight, weight)
...
#           return False
       if row.tPt < 30:  #was 35   #newcuts30
           return False
#       if row.tMtToPfMet_JetEnUp > 105:  #was 50   #newcuts65
#           return False
       if self.ls_recoilC and MetCorrection:
...
#	    return False
  ...
  #          return False
    ...
    def vbf_gg(self,row):
        ...
        if row.mPt < 25:   #was 40    #newcut 25
       		return False
   #     if row.tMtToPfMet_JetEnUp > 105: #was 35   #newcuts 55
   #             return False
        if self.ls_recoilC and MetCorrection:
            if self.tMtToPfMet_type1MetC > 105:  #was 50   #newcuts65
               return False
...
#	if(row.vbfNJets30_JetEnUp<2):
#	    return False
	#if (abs(row.vbfDeta_JetEnUp)>3.5 and row.vbfMass_JetEnUp > 550):   #was 2.5    #newcut 2.0
	if (row.vbfMass_JetEnUp > 550):   #was 2.5    #newcut 2.0
	    return False
#        if row.vbfMass_JetEnUp > 550:    #was 20   newcut 240
#	    return False
#        if row.vbfMass_JetEnUp < 100:    #was 20   newcut 240
#	    return False
  #      if row.vbfJetVeto30_JetEnUp > 0:
  #          return False
#        if row.bjetCISVVeto30Medium:
