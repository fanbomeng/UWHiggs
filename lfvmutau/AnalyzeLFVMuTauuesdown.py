...
#              print row.m_t_collinearmass_UnclusteredEnDown
             ...
             #     self.fill_histos(row,'gg',False)
              ...
              if  row.jetVeto30==0:
                  if RUN_OPTIMIZATION:
                     for  i in optimizer.compute_regions_0jet(row.tPt, row.mPt, deltaPhi(row.mPhi,row.tPhi),abs(row.tDPhiToPfMet_UnclusteredEnDown),row.tMtToPfMet_UnclusteredEnDown):
 ...
 #                              self.fill_histos(row,'ggTD10',False)
             ...
             #     self.fill_histos(row,'boost',False)
              if row.jetVeto30==1:
                  if RUN_OPTIMIZATION:
                     for  i in optimizer.compute_regions_1jet(row.tPt, row.mPt,deltaPhi(row.mPhi,row.tPhi),abs(row.tDPhiToPfMet_UnclusteredEnDown),row.tMtToPfMet_UnclusteredEnDown):
