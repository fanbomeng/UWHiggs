...
def fullMT(mupt,taupt , muphi, tauphi, row, sys='none'):
        ...
        met = row.type1_pfMet_shiftedPt_TauEnUp
        metphi = row.type1_pfMet_shiftedPhi_TauEnUp
...
class AnalyzeLFVMuTauPostBDT_progresstesup(MegaBase):
    tree = 'mt/final/Ntuple'
    #tree = 'New_Tree'

    def __init__(self, tree, outfile, **kwargs):
        super(AnalyzeLFVMuTauPostBDT_progresstesup, self).__init__(tree, outfile, **kwargs)
...
#          histos['jetPt'].Fill(1,1)
    def MetCorrectionSet(self,row):
           if self.ls_Wjets:
              self.tmpMet=self.Metcorected.CorrectByMeanResolution(row.type1_pfMet_shiftedPt_TauEnUp*math.cos(row.type1_pfMet_shiftedPhi_TauEnUp),row.type1_pfMetEt*math.sin(row.type1_pfMetPhi),row.genpX,row.genpY,row.vispX,row.vispY,int(round(row.jetVeto30))+1)#,self.pfmetcorr_ex,self.pfmetcorr_ey)
           else:
              self.tmpMet=self.Metcorected.CorrectByMeanResolution(row.type1_pfMet_shiftedPt_TauEnUp*math.cos(row.type1_pfMet_shiftedPhi_TauEnUp),row.type1_pfMetEt*math.sin(row.type1_pfMetPhi),row.genpX,row.genpY,row.vispX,row.vispY,int(round(row.jetVeto30)))#,self.pfmetcorr_ex,self.pfmetcorr_ey)
           ...
           print "Mass from Tuple %f and from Cal %f" %(row.type1_pfMet_shiftedPt_TauEnUp*math.cos(row.type1_pfMet_shiftedPhi_TauEnUp),self.tmpMet[0])
...
#        histos[name+'/mPtavColMass'].Fill(row.mPt/row.m_t_collinearmass_TauEnUp, weight)
        histos[name+'/mEta'].Fill(row.mEta, weight)
        histos[name+'/mMtToPfMet_type1'].Fill(row.mMtToPfMet_TauEnUp,weight)
...
#        histos[name+'/tPtavColMass'].Fill(row.tPt/row.m_t_collinearmass_TauEnUp, weight)
        ...
        if self.ls_recoilC and MetCorrection:
           ...
           histos[name+'/tMtToPfMet_type1'].Fill(row.tMtToPfMet_TauEnUp,weight)
...
#        histos[name+'/tLeadTrackPt'].Fill(row.tLeadTrackPt,weight)
     ...
     #      print "Mass from Tuple %f and from Cal %f" %(row.tMtToPfMet_TauEnUp,self.tMtToPfMet_type1MetC) 
     #      print "Mass from Tuple %f and from Cal %f" %(row.m_t_collinearmass_TauEnUp,self.collMass_type1MetC) 
        else:
           histos[name+'/collMass_type1'].Fill(row.m_t_collinearmass_TauEnUp,weight)
...
#        histos[name+'/collMass_type1MetC'].Fill(self.collMass_type1(row,tmpMet[0],tmpMet[1]),weight)
    #    print row.m_t_collinearmass_TauEnUp
        ...
        histos[name+'/type1_pfMetEt'].Fill(row.type1_pfMet_shiftedPt_TauEnUp,weight)

        histos[name+'/m_t_Mass'].Fill(row.m_t_Mass_TauEnUp,weight)
...
#	return True
    ...
    def gg(self,row):
       ...
       if row.tPt < 30:  #was 35   #newcuts30
           return False
       if row.tMtToPfMet_TauEnUp > 75:  #was 50   #newcuts65
           return False
       if abs(row.tDPhiToPfMet_TauEnUp)>3.0:
           return False
       if row.jetVeto30!=0:
           return False
    ...
    def boost(self,row):
          ...
          if row.tPt < 30:  #was 40  #newcut 30
                return False
          if row.tMtToPfMet_TauEnUp > 105: #was 35   #newcuts 75
                return False
          if abs(row.tDPhiToPfMet_TauEnUp)>3.0:
                return False
          if row.bjetCISVVeto30Loose:
    ...
    def vbf(self,row):
       ...
       # if row.mPt < 30:
       # 	return False
        if row.tMtToPfMet_TauEnUp > 75: #was 35   #newcuts 55
                return False
        if row.jetVeto30<2:  
            return False
#	if(row.vbfNJets<2):
...
#	    return False
    ...
    def vbf_vbf(self,row):
        ...
        if row.mPt < 25:   #was 40    #newcut 25
                return False
  #      if row.tMtToPfMet_TauEnUp > 85: #was 35   #newcuts 55
  #              return False
...
#                return False
        if self.ls_recoilC and MetCorrection:
            ...
            if row.tMtToPfMet_TauEnUp > 85:  #was 50   #newcuts65
               return False
#        if row.jetVeto30<2:
#            return False
#        if(row.vbfNJets30<2):
...
#            return False
    ...
    def WjetsEnrich(self,row):
        if (row.tMtToPfMet_TauEnUp>60 and row.mMtToPfMet_TauEnUp>80):
            return True
        return False
...
#            self.var_d_0  ={'mPt_':row.mPt,'tPt_':row.tPt,'mMtToPfMet_type1_':row.mMtToPfMet_TauEnUp,'tMtToPfMet_type1_':row.tMtToPfMet_TauEnUp,'m_t_DPhi_':row.m_t_DPhi,'tDPhiToPfMet_type1_':row.tDPhiToPfMet_TauEnUp,'type1_pfMetEt_':row.type1_pfMet_shiftedPt_TauEnUp,'jetVeto30_':row.jetVeto30,'vbfMass_':-0.1,'vbfDeta_':-0.1,'deltaeta_m_t_':row.mEta-row.tEta} 
            self.var_d_0  ={'mPt_':row.mPt,'tPt_':row.tPt,'tMtToPfMet_type1_':row.tMtToPfMet_TauEnUp,'m_t_DPhi_':abs(row.m_t_DPhi),'tDPhiToPfMet_type1_':row.tDPhiToPfMet_TauEnUp,'type1_pfMetEt_':row.type1_pfMet_shiftedPt_TauEnUp,'deltaeta_m_t_':abs(row.mEta-row.tEta),'m_t_collinearmass_':row.m_t_collinearmass_TauEnUp}
            self.var_d_1  =self.var_d_0 
            self.var_d_2  =self.var_d_0
           # self.var_d_2  ={'mPt_':row.mPt,'tPt_':row.tPt,'mMtToPfMet_type1_':row.mMtToPfMet_TauEnUp,'tMtToPfMet_type1_':row.tMtToPfMet_TauEnUp,'m_t_DPhi_':row.m_t_DPhi,'tDPhiToPfMet_type1_':row.tDPhiToPfMet_TauEnUp,'type1_pfMetEt_':row.type1_pfMet_shiftedPt_TauEnUp,'jetVeto30_':row.jetVeto30,'vbfMass_':row.vbfMass/2600.0,'vbfDeta_':row.vbfDeta/10.0,'deltaeta_m_t_':row.mEta-row.tEta} 
            #self.var_d_2  ={'mMtToPfMet_type1_':row.mMtToPfMet_TauEnUp,'tMtToPfMet_type1_':row.tMtToPfMet_TauEnUp,'m_t_DPhi_':row.m_t_DPhi,'tDPhiToPfMet_type1_':row.tDPhiToPfMet_TauEnUp,'type1_pfMetEt_':row.type1_pfMet_shiftedPt_TauEnUp,'jetVeto30_':row.jetVeto30,'vbfMass_':row.vbfMass,'vbfDeta_':row.vbfDeta,'deltaeta_m_t_':row.mEta-row.tEta} 
#            if row.jetVeto30==0:
...
#               MVA2=self.functor(**self.var_d_2)
            ...
            #        if MVA0>-0.01: 
                    ...
                    if RUN_OPTIMIZATION:
                       for  i in optimizer_new.compute_regions_0jet(row.tPt, row.mPt, deltaPhi(row.mPhi,row.tPhi),abs(row.tDPhiToPfMet_TauEnUp),row.tMtToPfMet_TauEnUp):
             ...
             #       if MVA0>-0.01:
                    ...
                    if RUN_OPTIMIZATION:
                       for  i in optimizer_new.compute_regions_1jet(row.tPt, row.mPt,deltaPhi(row.mPhi,row.tPhi),abs(row.tDPhiToPfMet_TauEnUp),row.tMtToPfMet_TauEnUp):
              ...
              #      if MVA0>-0.01:
                    ...
                    if RUN_OPTIMIZATION:
                          for  i in optimizer_new.compute_regions_2jettight(row.tPt, row.mPt,row.tMtToPfMet_TauEnUp,row.vbfMass,row.vbfDeta):
                    ...
                    if RUN_OPTIMIZATION:
                       for  i in optimizer_new.compute_regions_2jetloose(row.tPt, row.mPt,row.tMtToPfMet_TauEnUp,row.vbfMass,row.vbfDeta):
...
#              print row.m_t_collinearmass_TauEnUp
             ...
             #     self.fill_histos(row,'gg',False)
              ...
              if (row.jetVeto30==2) :
#                  if RUN_OPTIMIZATION:
#                     for  i in optimizer.compute_regions_2jet(row.tPt, row.mPt,row.tMtToPfMet_TauEnUp,row.vbfMass,row.vbfDeta):
...
#                 self.fill_histos(row,'notIsoNotWeighted',False)
           ...
           #          self.fill_histos(row,'ggNotIso',True)
                 if row.jetVeto30==1:
                  #  # if RUN_OPTIMIZATION:
                  #  #    for  i in optimizer.compute_regions_1jet(row.tPt, row.mPt,row.tMtToPfMet_TauEnUp):
...
#                 self.fill_histos(row,'notIsoNotWeighted',False)
                 ...
                 if  row.jetVeto30==0:
                    # #if RUN_OPTIMIZATION:
                       # for  i in optimizer.compute_regions_0jet(row.tPt, row.mPt, deltaPhi(row.mPhi,row.tPhi),row.tMtToPfMet_TauEnUp):
                    # #   for  i in optimizer.compute_regions_0jet(row.tPt, row.mPt, deltaPhi(row.mPhi,row.tPhi),abs(row.mDPhiToPfMet_TauEnUp),abs(row.tDPhiToPfMet_TauEnUp),row.tMtToPfMet_TauEnUp):
