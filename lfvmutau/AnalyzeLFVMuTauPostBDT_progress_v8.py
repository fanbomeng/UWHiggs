...
class AnalyzeLFVMuTauPostBDT_progress_v8(MegaBase):
    tree = 'mt/final/Ntuple'
    #tree = 'New_Tree'

    def __init__(self, tree, outfile, **kwargs):
        super(AnalyzeLFVMuTauPostBDT_progress_v8, self).__init__(tree, outfile, **kwargs)
...
#          print self.fakeRateMethod(row,fakeset)
    ...
    #    if name=='ggPileupUp':
    #       print 'the up pile up weight' 
    #       print weight
    #    if name=='ggPileupDown':
    #       print 'the down pile up weight '
    #       print weight
    #    if name=='gg':
    #       print 'the normal pile up weight '
    #       print weight
     #   if self.ls_DY or self.ls_ZTauTau:
...
#           print "metlorenz %f %f %f" %(metlorenz.Pt(),metlorenz.Px(),metlorenz.Py())
    ...
    def VariableCalculate(self,row,tau_Pt_C,MET_tPtC):
           ...
           if self.DoUES==1:
              if (not self.ls_DY) and row.tZTTGenMatching==5:
                 ...
                 if tmptDPhiToPfMet_type1>math.pi:
                    tmptDPhiToPfMet_type1=2*math.pi-tmptDPhiToPfMet_type1
                 var_d_0  ={'mPt_':row.mPt,'tPt_':row.tPt,'tMtToPfMet_type1_':tMtToPfMet_type1_new,'m_t_DPhi_':abs(row.m_t_DPhi),'tDPhiToPfMet_type1_':tmptDPhiToPfMet_type1,'type1_pfMetEt_':tmpHulaUESUp[0],'deltaeta_m_t_':abs(row.mEta-row.tEta),'m_t_collinearmass_':collMass_type1_new}
           ...
           if self.DoUES==2:
              if (not self.ls_DY) and row.tZTTGenMatching==5:
                 ...
                 if tmptDPhiToPfMet_type1>math.pi:
                    tmptDPhiToPfMet_type1=2*math.pi-tmptDPhiToPfMet_type1
                 var_d_0  ={'mPt_':row.mPt,'tPt_':row.tPt,'tMtToPfMet_type1_':tMtToPfMet_type1_new,'m_t_DPhi_':abs(row.m_t_DPhi),'tDPhiToPfMet_type1_':tmptDPhiToPfMet_type1,'type1_pfMetEt_':tmpHulaUESDown[0],'deltaeta_m_t_':abs(row.mEta-row.tEta),'m_t_collinearmass_':collMass_type1_new}
                 MVA0fill_new=self.functor(**var_d_0)
                 return(MVA0fill_new,collMass_type1_new,tMtToPfMet_type1_new,mMtToPfMet_type1_new,tmpHulaUESDown[0])
    def VariableCalculateJES_UESsp(self,row,JESShiftedMet,JESShiftedPhi,tau_Pt_C): 
           ...
           if (not self.ls_DY) and row.tZTTGenMatching==5:
              ...
              if tmptDPhiToPfMet_type1>math.pi:
                 tmptDPhiToPfMet_type1=2*math.pi-tmptDPhiToPfMet_type1
              var_d_0  ={'mPt_':row.mPt,'tPt_':row.tPt,'tMtToPfMet_type1_':tMtToPfMet_type1_new,'m_t_DPhi_':abs(row.m_t_DPhi),'tDPhiToPfMet_type1_':tmptDPhiToPfMet_type1,'type1_pfMetEt_':metlorenz.Pt(),'deltaeta_m_t_':abs(row.mEta-row.tEta),'m_t_collinearmass_':collMass_type1_new}
                    ...
                    for L in sysneedMFT:
                        ...
                        if row.isZmumu  and row.tZTTGenMatching<5 and row.tDecayMode==1:
                         ...
                         #  cheksiski=False
                           exec("%s = %d" % (tmpname,3))
                        if self.obj2_iso(row) and self.oppositesign(row) and self.obj1_iso(row) and self.kinematics(row):
                           self.MVA0fill_new,self.collMass_type1_new,self.tMtToPfMet_type1_new,self.MorTPtshifted=self.VariableCalculate(row,self.tau_Pt_C,self.MET_tPtC)
                           for j in basechannelsI:
                              if  row.jetVeto30==j[0]:
                                  tmpname_2='self.'+j[1]+'(row)'
                                  if eval(tmpname_2):
