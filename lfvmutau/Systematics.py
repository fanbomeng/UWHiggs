...
def TESup(mytau,mymet,mytauphi,mymetphi):
    ...
    mex = mymet*math.cos(mymetphi)-0.03*(tauex/1.03)
    mey = mymet*math.sin(mymetphi)-0.03*(tauey/1.03)
...
def TESdown(mytau,mymet,mytauphi,mymetphi):
    ...
    tauex=mytau*math.cos(mytauphi)
    tauey=mytau*math.sin(mytauphi)
    mex = mymet*math.cos(mymetphi)+0.03*(tauex/0.97)
    mey = mymet*math.sin(mymetphi)+0.03*(tauey/0.97)
    return (mytau,tauex,tauey,abs(math.sqrt(mex*mex+mey*mey)),mex,mey)
#else if (TESdown){
#    mytau=0.97*mytau;
...
def UESup(met_UESup,metphi_UESup):
    ...
    return (met_UESup,met_UESup_x,met_UESup_y,metphi_UESup)
def UESdown(met_UESdown,metphi_UESdown):
    ...
    return (met_UESdown,met_UESdown_x,met_UESdown_y,metphi_UESdown)
#else if (METESup){
...
def MESup(mymu,mymueta,mymet,mymuphi,mymetphi):
    if abs(mymueta)<=1.2:
       ...
       mex = mymet*math.cos(mymetphi)-0.004*(muex/1.004)
       mey = mymet*math.sin(mymetphi)-0.004*(muey/1.004)
...
def MESdown(mymu,mymueta,mymet,mymuphi,mymetphi):
    if abs(mymueta)<=1.2:
       ...
       muex=mymu*math.cos(mymuphi)
       muey=mymu*math.sin(mymuphi)
       mex = mymet*math.cos(mymetphi)+0.004*(muex/0.996)
       mey = mymet*math.sin(mymetphi)+0.004*(muey/0.996)
    if abs(mymueta)>1.2 and abs(mymueta)<=2.1 :
       mymu=0.991*mymu
       muex=mymu*math.cos(mymuphi)
       muey=mymu*math.sin(mymuphi)
